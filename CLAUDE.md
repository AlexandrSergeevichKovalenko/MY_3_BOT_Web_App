...
- `git push` запускает `backend/tests` (~50 c). Красное — пуш не уходит.
  Срочно и осознанно: `git push --no-verify`.
...
- **Тесты не должны трогать боевую базу.** Импорт приложения делает `ensure_*_schema`,
  ...
  ставит `SKIP_STARTUP_SCHEMA_BOOTSTRAP=1`. В проде эта переменная НЕ ставится.
