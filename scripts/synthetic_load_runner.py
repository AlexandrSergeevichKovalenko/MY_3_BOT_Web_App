...
# ┌─ ЗАМЕРЕНО 06.09.2026 НА ИЗОЛИРОВАННОМ СТЕНДЕ. НЕ ПЕРЕМЕРЯТЬ БЕЗ ПОВОДА. ────────────┐
# │ ЗАКАЗ ВЛАДЕЛЬЦА: «положат ли мне бот 200–500 пользователей». Стенд: локальный        │
# │ docker-compose.synthetic.yml (Postgres+PgBouncer transaction+Redis), обработчик      │
# │ делает access-check из кеша и один SELECT 1 через пул. Ноутбук 12 ядер Apple.        │
# │                                                                                      │
# │ ЛЕСТНИЦА (500 виртуальных людей, 30 с на ступень, пул бота 8, PgBouncer 16):         │
# │   заказ/с   ФАКТ/с   обработчик сред   «пул кончился»                                │
# │        20       19            6.4 мс                0                                │
# │       100       91            4.1 мс                0                                │
# │       200      184            3.4 мс                3                                │
# │       250      232            3.3 мс                6                                │
# │       300      274          112.7 мс              933   ← колено                     │
# │       350      299          540.7 мс             2781                                │
# │ Ошибок НОЛЬ на всех ступенях, включая 800: система замедляется, а не падает.         │
# │                                                                                      │
# │ ⛔ ГРАНИЦА ИНСТРУМЕНТА: шаг планировщика = 1/rate, и asyncio.sleep не держит шаг      │
# │ мельче ~3 мс. Выше ~370/с диспетчер меряет САМ СЕБЯ, а не систему. Числа за 350      │
# │ заказанных смотреть ТОЛЬКО по фактическому completed_tasks/elapsed_sec.              │
# │                                                                                      │
# │ ⛔ ГЛАВНАЯ НАХОДКА, ПРОТИВОРЕЧИТ ЗДРАВОМУ СМЫСЛУ — ПРОВЕРЕНА ДВАЖДЫ:                  │
# │ БОЛЬШЕ ПУЛ = МЕНЬШЕ ПРОПУСКНАЯ СПОСОБНОСТЬ.                                          │
# │   пул  8, PgBouncer  16 → 274/с факт, обработчик  113 мс, «пул кончился» 933         │
# │   пул 16, PgBouncer  16 → 170/с факт, обработчик 1682 мс, «пул кончился»   0         │
# │   пул 16, PgBouncer 128 → 156/с факт, обработчик 1841 мс, «пул кончился»   0         │
# │ Удвоение пула ПОЧТИ ВДВОЕ УРЕЗАЛО пропускную способность и подняло задержку в 15    │
# │ раз. Расширение PgBouncer поверх этого не дало НИЧЕГО. Значит пул на 8 работает не   │
# │ ограничением, а ОГРАНИЧИТЕЛЕМ: держит очередь на входе, где она дешёвая и видимая.   │
# │ Отсюда же: совет «поднять DB_POOL_MAXCONN» в письме о голоде пула был вреден — это   │
# │ теперь не рассуждение, а замер. См. рамку в backend/database.py у                    │
# │ _build_db_pool_starvation_message.                                                   │
# │                                                                                      │
# │ ЧЕГО ЭТОТ ЗАМЕР НЕ ГОВОРИТ (не выдавать за «выдержим»):                              │
# │  • обработчик делает SELECT 1 — это потолок ВОДОПРОВОДА. Настоящие обработчики       │
# │    тяжелее, поэтому реальный предел НИЖЕ измеренного, а не равен ему;                │
# │  • ноутбук 12 ядер Apple против 8 общих ядер контейнера бота — в проде колено ниже;  │
# │  • одиночный процесс run_polling как архитектурный потолок здесь не проверялся.      │
# │                                                                                      │
# │ Как перемерить: docker compose -f docker-compose.synthetic.yml up -d, затем          │
# │ scripts/synthetic_load_dispatch.py --users 500 --rate N --duration 30 --execute       │
# │ --concurrent --max-in-flight 300 с DATABASE_URL* на 127.0.0.1:56432 (?sslmode=disable│
# │ обязателен: у стенда нет TLS) и SYNTHETIC_LOAD_MODE=1.                               │
# └──────────────────────────────────────────────────────────────────────────────────────┘
