...
def _ru_key(value: str) -> str:
    """Ключ сравнения русских переводов: без регистра и без концевого знака."""
            ...
            for review_id, unit_id, old_text, saved_ru, judges, arbiter, status in rows:
                ...
                if status == "accepted":
                    ...
                    if not button_ru:
                        ...
                        # Русский сравниваем БЕЗ регистра и БЕЗ концевого знака, в
                        # отличие от немецкого. В немецком регистр — грамматика («Es ist
                        # mir Latte» → «latte» и есть вся правка). В русском переводе
                        # заглавная в начале и запятая в конце — оформление: ensure_unit
                        # переиспользует уже лежащую единицу «я умираю от жары», и это
                        # тот же перевод, а не подмена. Без регистра отчёт насчитал 8
                        # подмен там, где их 2; без концевого знака осталась одна ложная —
                        # #197, где на кнопке стояло «…холодных гостиных,» с запятой, а в
                        # базе лежит та же строка без неё (запятую снял clean_text ещё у
                        # немецкого). Правило отбора то же, что в
                        # scripts/phrase_review_screen_check.py — два отчёта об одном
                        # обязаны считать одинаково.
                        if _ru_key(top_ru) != _ru_key(button_ru):
