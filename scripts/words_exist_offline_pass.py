...
# ── ЧТО СПРАВОЧНИК НЕ ХРАНИТ ПО ПОСТРОЕНИЮ ───────────────────────────────────
#
# Решение владельца 25.08.2026. Разбор всех 143 «неизвестных» слов показал: настоящих
# среди них 140. Справочник молчал не потому, что слова нет, а потому что в офлайн-
# выгрузке лежат ТОЛЬКО существительные (89 681), глаголы (1 499) и прилагательные
# (5 319). Союз «weil», наречие «herunter», предлог «außer», причастие «gefährdet»,
# артикль «des» туда не входят и никогда не войдут.
#
# Пока этого списка не было, они всплывали подозрительными в КАЖДОМ прогоне, и мы
# разбирали их заново. Владелец, дословно: «решить один раз навсегда, а не мусолить
# каждый день». Вот и решение: такие части речи в кучу «не знаем» не попадают.
СПРАВОЧНИК_ИХ_НЕ_ХРАНИТ = frozenset({
    "conjunction",        # weil, dass, obwohl, falls, da, wohingegen
    "preposition",        # außer, von
    "lokaladverb",        # herauf, heraus, herein, herüber, herunter, hinauf, hinaus
    "pronominaladverb",   # dadurch
    "temporaladverb",     # derzeit
    "konjunktionaladverb",  # dennoch
    "adverb",
    "pronoun",            # nix, was
    "numeral",            # anderthalb, tausende
    "article",            # des
    "interjection",       # jein, danke
    "particle",
    "participle",         # abweisend, angedeutet, gefährdet — формы глагола, не леммы
})

# ── ЧУЖИЕ СЛОВА, КОТОРЫЕ ВЛАДЕЛЕЦ РЕШИЛ ОСТАВИТЬ ─────────────────────────────
#
# Решение владельца 25.08.2026: «keep it, but mark». Это английские слова в немецком
# словаре; человек сохранил их осознанно, и его карточку мы не трогаем. Но и
# подозрительными они больше не считаются — вопрос закрыт, к нему не возвращаемся.
ЧУЖИЕ_ОСТАВЛЕНЫ = frozenset({"know-it-all", "sticky", "including", "stick-in-the-mud"})
...
def main() -> int:
    ...
    for uid, display, pos in слова:
        ...
        elif low in ЧУЖИЕ_ОСТАВЛЕНЫ:
            # Решение владельца принято, слово живёт. В «не знаем» ему не место.
            оставлены.append(голое)
        elif str(pos or "").strip().lower() in СПРАВОЧНИК_ИХ_НЕ_ХРАНИТ:
            # Не «слова нет», а «мы искали не в том ящике»: союзов и наречий в выгрузке
            # существительных не бывает. Считаем отдельно, чтобы не путать с браком.
            вне_охвата.append((голое, pos))
        else:
            не_знаем.append((uid, голое, pos))

    всего = len(есть) + len(форма_другого) + len(не_знаем) + len(вне_охвата) + len(оставлены)
    print()
    ...
    print(f"  вне охвата справочника:  {len(вне_охвата):5}  — союзы, наречия, предлоги: их там и нет")
    print(f"  чужие, решение принято:  {len(оставлены):5}  — владелец оставил 25.08.2026")
