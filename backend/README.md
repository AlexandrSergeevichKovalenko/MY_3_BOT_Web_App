...
- `BACKEND_WEB` -> `backend/web_service.py` -> `backend/backend_server.py`
...
| --- | --- | --- |
| `BACKEND_WEB` | `backend/web_service.py` -> `backend/backend_server.py` | Flask route, auth, orchestration, sync/async decision making; web tier stays lean and keeps runtime side effects off by default |
