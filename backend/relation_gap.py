...
def build_gap_items_for(item: dict) -> tuple[list, dict]:
    """ЕДИНСТВЕННАЯ дверь сборки задания по строке банка. Все зовут её.

    ┌─ СВЕДЕНО В ОДНО ПРАВИЛО 15.09.2026 ПО РЕШЕНИЮ ВЛАДЕЛЬЦА. ─────────────────────┐
    │ Правил было два, и они расходились в самом опасном месте. Экран человека      │
    │ (answer_eval.load_gap_task), проверка его ответа и обещание строили заготовки │
    │ СО справочником спряжений, а ОТПРАВИТЕЛИ (рассылка по часам и капля) — без    │
    │ него. Отправитель строит заготовки ровно затем, чтобы решить «есть ли что     │
    │ показывать»; по бедному правилу он видел ноль и молча не отправлял задание,   │
    │ которое у человека на экране собралось бы полностью.                          │
    │                                                                               │
    │ ЗАМЕР 15.09.2026 на живом банке (77 слов): без справочника 475 заготовок, со  │
    │ справочником 485. Ровно ТРИ слова давали ноль против «есть»: verwirren (4),   │
    │ bemerken (2), versäumen (4) — у всех синонимы-глаголы, а немецкий глагол       │
    │ спрягается заменой «-en», не приклейкой окончания. Слов, где отправитель видит│
    │ МЕНЬШЕ, но не ноль, — нет ни одного, поэтому вреда от сведения быть не может:  │
    │ меняется только «отправлять или нет», и меняется в сторону правды.            │
    │ Цена: один запрос к справочнику на отправку слота (не на человека).           │
    │                                                                               │
    │ Перемерить: build_gap_items по всему банку с forms_of и без — числа выше.     │
    └───────────────────────────────────────────────────────────────────────────────┘

    `item` — строка банка целиком (wort, accepted, trainer_json): справочник форм
    ищется по её же примерам, поэтому вызывающему не нужно знать про справочник.
    """
    # Ленивый импорт: сам файл остаётся без базы и без сети (его правила проверяются
    # тестами), а справочник спряжений живёт в базе.
    from backend.answer_eval import _gap_forms_lookup
    return build_gap_items(
        wort=str((item or {}).get("wort") or ""),
        accepted=(item or {}).get("accepted"),
        trainer_json=(item or {}).get("trainer_json") or {},
        forms_of=_gap_forms_lookup(item or {}),
    )
