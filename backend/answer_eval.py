...
# │ До этой строки `logging` в файле НЕ импортировался на уровне модуля: его тянули    │
# │ внутрь отдельных функций (_run, _spawn_listening_grader, load_artikel_review_batch),│
# │ а `import` внутри функции виден ТОЛЬКО в ней. Разбор 13.09.2026 по всему файлу:    │
# │ три функции звали logging, не импортировав его в своей области — evaluate_aufgabe  │
# │ (:2533), evaluate_sprint (:2873) и load_gap_task. Все три строки стоят в обработке │
# │ ошибок, то есть ровно там, где надо было тихо записать «не смогли»: вместо записи  │
# │ приложение падало с NameError и отдавало человеку 500.                             │
# │ Поймано в проде: владелец открыл «Подставь синоним» на слове sich erinnern, у      │
# │ которого 4 заготовки не построились → сработала строка со счётчиком отказов →      │
# │ экран «Не удалось загрузить». У антонима отказов не было, и он открывался.         │
# │ Перемерить: python3 -c по ast — функции, где logging.* есть, а import logging в их │
# │ области нет; ждём 0. Класс закрыт тестом test_answer_eval_logging_imported.        │
# └────────────────────────────────────────────────────────────────────────────────────┘
import logging
import re
