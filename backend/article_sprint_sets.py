...
def build_practice_set(theme_key: str, user_id: int, play_date, *, size: int = PRACTICE_SET_SIZE) -> dict:
    ...
    uniq = _dedup_words(words)
    # ПЕРЕМЕШИВАТЬ ЗДЕСЬ НЕЛЬЗЯ. База отдаёт слова в порядке `cooling, random()`:
    # внутри каждой группы порядок уже случайный, а сами группы стоят так, что
    # недавно выученное лежит в конце очереди. `random.shuffle` эту работу
    # выбрасывал целиком.
    #
    # Почему это важно, хотя набор всё равно содержит всю тему. За две минуты
    # человек доходит примерно до 72-го слова (замер 19.08.2026), а в теме их,
    # например, 118 — значит решает НЕ состав набора, а порядок. Замер на живых
    # данных владельца, тема «Кухня и посуда»: из первых 72 слов знакомых было 17,
    # после перемешивания стало 21. Сегодня разница небольшая, потому что в окне
    # остывания у него всего 11 слов из 118 — но чем больше человек выучил, тем
    # больше эта строка у него отнимает: у того, кто знает половину темы, она
    # возвращает половину повторов обратно в начало игры.
