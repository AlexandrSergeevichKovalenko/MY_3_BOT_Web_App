...
> **Статус (2026-07-01):** редизайн уже внедрён в прод. Что отличается от инструкций ниже:
>
> - Вся JSX-логика читалки вынесена из `App.jsx` в отдельный компонент
>   **`frontend/src/components/ReaderSection.jsx`** (импортируется в `App.jsx`,
>   строка 7; рендерится около строки 34115). Часть reader-state (в т.ч. тема)
>   по-прежнему живёт в `App.jsx` и прокидывается пропсами.
> - `reader-redesign.css` **уже импортирован** — `frontend/src/App.jsx`, строка 4
>   (сразу после `App.css`). Раздел «Установка» ниже — исторический.
> - Патчи Фазы 2 **2.1 (Hero-карточка)** и **2.2 (Темы чтения)** уже реализованы
>   в `ReaderSection.jsx` (плюс появились сегментированные контролы «Разметка»
>   и «Движок озвучки»). Оставлены ниже как справка «как устроено / как откатить».
> - Патч **2.4 (аудио-чтение с подсветкой)** тоже фактически уехал: бэкенд хранит
>   word-таймкоды в `bt_3_reader_audio_pages.word_timings` (аудио — mp3
>   `audio/mpeg`, не WAV), а во фронте есть karaoke-подсветка.
> - Не сделан только патч **2.3 (список закладок и заметок)** — в БД по-прежнему
>   одна `bookmark_percent` на книгу.
...
> ✅ **Реализовано** — в `frontend/src/components/ReaderSection.jsx` (~строка 321).
> Блок ниже оставлен как справка по устройству.
...
> ✅ **Реализовано.** State (`readerColorTheme` + `applyReaderColorTheme` +
> `localStorage['reader_color_theme']`) живёт в `App.jsx` (~строки 5454 и 5491);
> сам сегментированный контрол — в Settings sheet внутри `ReaderSection.jsx`,
> а `data-reader-theme` навешивается на корневую `<section>` читалки там же
> (для библиотеки тема принудительно `dark`). Блок ниже — справка по устройству.
...
Около строки **5454** (рядом с другими reader-state'ами), добавьте:
...
> ✅ **Уже сделано.** Бэкенд перешёл на постраничное mp3-аудио (`audio/mpeg`) с
> word-level таймкодами, которые хранятся в `bt_3_reader_audio_pages.word_timings`
> (JSONB со `start_ms`/`end_ms`/`char_start`), а во фронте есть karaoke-подсветка
> (`App.jsx`, поиск по `word_timings`). Абзац ниже описывает изначальный блокер.

Изначально API возвращало целый WAV без word-таймкодов. Для синхронной подсветки
нужны были таймкоды слов/предложений от TTS. После перехода на TTS-движок
с word-level timing (Google TTS SSML marks / `word_timings`) UI стал возможен.
