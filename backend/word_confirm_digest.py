...
# ┌─ О ЧЁМ МЫ ЧЕЛОВЕКА БОЛЬШЕ НЕ СПРАШИВАЕМ. Решение владельца 04.09.2026. ───────────┐
# │ «Зачем мне это приходит и пользователей беспокоит напрасно, если слово настоящее, │
# │ просто его нет в справочнике??»                                                   │
# │                                                                                  │
# │ Класс «модель: слово есть, справочник не знает» — это НЕ сомнение. Мы спросили     │
# │ модель, она ответила «слово настоящее, немецкое», и после этого шли к человеку с   │
# │ кнопкой «Удалить». Спрашивать нечего: ответ у нас уже есть.                        │
# │                                                                                  │
# │ Причём это устройство языка, а не дефект слова: немецкие сложные слова склеиваются │
# │ бесконечно, и ни один справочник их все не держит. Замер 04.09.2026 на живой базе: │
# │ на экране владельца стояло 16 слов, из них 13 — этого класса (Massenimmigration,   │
# │ Puzzlefragment, Hauptverschwörer, Attentatsopfer, Verschwörungsmitglied…), то есть │
# │ 4 беспокойства из 5 были ни о чём. Всего в журнале двери таких 24.                 │
# │                                                                                  │
# │ ЧТО ОСТАЁТСЯ НА ЭКРАНЕ: обрубки («модель: такого слова нет»), чужие языки          │
# │ («модель: слово есть, язык en» — Sweatpants) и спорное написание. Английские слова │
# │ владелец решил оставить на экране в тот же день, отдельным ответом.                │
# │                                                                                  │
# │ Статус в базе НЕ трогаем: «не подтверждено» — правда, справочник это слово и       │
# │ вправду не знает. Меняется только адресат: это наша забота, а не человека.         │
# │ Фильтр живёт ОДНОЙ строкой на все три выборки модуля — список экрана, список для   │
# │ письма и подсчёт получателей. Разойдутся они — и письмо снова позовёт на экран,    │
# │ где показывать нечего (ровно этот дефект чинили 26.08.2026 и 04.09.2026).          │
# │ Перемерить: SELECT source, count(*) FROM bt_3_word_check                           │
# │             WHERE status IN ('не подтверждено','не слово') GROUP BY 1;             │
# └──────────────────────────────────────────────────────────────────────────────────┘
_НЕ_СПРАШИВАЕМ = "w.source <> 'модель: слово есть, справочник не знает'"
...
# С 31.08.2026 четвёртый вид — 'personal': панель нашла ошибку в САМОЙ фразе или в её
# переводе, а писал их человек. Переписывать его текст молча мы не имеем права
# (правило владельца 23.08.2026), поэтому вопрос адресован ему: вот что нашли, вот как
# правильно, решай. До этого дня такая находка ложилась отметкой в базу и умирала там —
# 384 карточки, и человек продолжал учить неверное.
ВИДЫ_ДЛЯ_ЧЕЛОВЕКА = ["grammar", "panel", "personal"]

# ⛔ ПОРЦИЯ ЛИЧНЫХ ВОПРОСОВ ЗА РАЗ. Решение владельца 31.08.2026: «давай по 20 за раз
# высылать пользователю».
#
# ПОЧЕМУ ИМЕННО ЭТОТ ВИД. Пересуд накопленного дал 328 вопросов «ошибка в твоём тексте»
# одному человеку — на экране, где до этого было 105 карточек. Вывалить их разом значит
# показать стену: человек закроет её и не разберёт ни одной. Остальные виды вопросов
# идут как шли — их поток и так небольшой.
#
# ⚠ ЭТО НЕ ФИЛЬТР «НАВСЕГДА», А ОКНО. Неразобранные не исчезают и не помечаются: в
# следующем письме придут те же двадцать, а как только человек их решит — следующие.
# Порядок стабильный (по номеру вопроса), поэтому очередь именно двигается, а не
# перетасовывается каждый раз заново.
ЛИЧНЫХ_ЗА_РАЗ = int(os.getenv("PERSONAL_QUESTIONS_PER_DIGEST", "20") or "20")
...
def _phrase_items(cur, user_id: int, limit: int) -> list[dict[str, Any]]:
    ...
    for (review_id, текст, перевод, судьи, арбитр,
        ...
        # Порция: больше двадцати личных вопросов за раз человеку не показываем.
        # Остальные не тронуты и придут следующими — см. ЛИЧНЫХ_ЗА_РАЗ.
        if str(вид or "") == "personal":
            личных_показано += 1
            if личных_показано > ЛИЧНЫХ_ЗА_РАЗ:
                continue
        # ЛИЧНЫЙ ВОПРОС: панель нашла ошибку в его собственном тексте.
        # Претензия к ПЕРЕВОДУ правится там же, где перевод и живёт, — в карточке
        # (поле перевода и примеры). Претензия к САМОЙ ФРАЗЕ — это вопрос о фразе:
        # там кнопка «да, правильно так» переписывает заголовок статьи.
        личный = str(вид or "") == "personal"
        готовое = _готовый_вариант(судьи) if личный else {}
        # Вид карточки определяет ПРЕТЕНЗИЯ, а не наличие готового варианта: у старых
        # отметок варианта нет вовсе, но претензия к переводу всё равно правится
        # полем перевода, а не переименованием фразы.
        # Поле берём У ТОЙ ЖЕ претензии, чья кнопка стоит на экране: иначе человек
        # видит карточку с полем перевода, а кнопка под ней переписывает саму фразу.
        # Кнопки нет — берём первую претензию.
        поле = (готовое.get("field") or _поле_претензии(судьи)) if личный else ""
        правим_карточку = панель or (личный and поле == "translation")
        items.append({
            "word": str(текст),
            "translation": str(перевод),
            "status": "фраза",
            "why": _личная_причина(поле) if личный else _phrase_reason(судьи),
            "variants": ([] if (панель or личный)
                         else кнопки_вариантов(судьи, str(текст), арбитр)),
            ...
            "judges": [] if (панель or личный) else _слова_судей(судьи),
            "arbiter": None if (панель or личный) else _слова_арбитра(арбитр),
            # Предмет панельного спора: претензии по пунктам и сами примеры.
            "doubts": _претензии(судьи) if (панель or личный) else [],
            "examples": (phrase_review_card_examples(карточка)
                         if правим_карточку else []),
            "question": "card" if правим_карточку else "text",
            # Готовый вариант от судьи — кнопка в одно касание. Пусто — значит его
            # никто не назвал: тогда у человека остаются «оставить как есть», «свой
            # вариант» и «удалить», а выдумывать за судью мы не станем.
            "fix": готовое.get("fix", ""),
            "fix_field": готовое.get("field", ""),
            # Чей это вопрос по существу: о наполнении карточки (панель) или о тексте,
            # который писал сам человек. Экран подписывает их разными словами.
            "personal": личный,
            "suggestion": "",
            "safe": False,
            "kind": "phrase",
...
def _поле_претензии(судьи: Any) -> str:
    """О чём претензия: «headword» (сама фраза) или «translation» (перевод). Иначе «»."""
    for судья in (судьи if isinstance(судьи, list) else []):
        if not isinstance(судья, dict):
            continue
        поле = str(судья.get("field") or "").strip()
        if поле in ("headword", "translation"):
            return поле
    return ""


def _личная_причина(поле: str) -> str:
    """Почему мы спрашиваем — своими словами, без вранья про «разошлись».

    ⚠ ОБЩАЯ ПРИЧИНА ЗДЕСЬ НЕ ГОДИТСЯ. `_phrase_reason` для незнакомой категории пишет
    «Проверяющие разошлись во мнении об этой фразе». В личном вопросе они как раз
    СОШЛИСЬ — двое из трёх назвали одну и ту же ошибку. Написать «разошлись» значит
    соврать человеку о том, насколько мы уверены.
    """
    if поле == "translation":
        return ("Проверяющие считают, что этот перевод не соответствует немецкому. "
                "Перевод твой — решаешь ты.")
    return ("Проверяющие считают, что так по-немецки не говорят. "
            "Фраза твоя — мы её не переписываем, решаешь ты.")


def _готовый_вариант(судьи: Any) -> dict[str, str]:
    """Первый готовый вариант, который МОЖНО показать кнопкой. Иначе пустой словарь.

    ⚠ ЗАБРАКОВАННЫЙ ВТОРЫМ ГОЛОСОМ ВАРИАНТ ЧЕЛОВЕКУ НЕ ПОКАЗЫВАЕМ. На его экране нет
    места для возражения проверки, а кнопка «да, правильно так» читается как «система
    уверена». Отдать одним касанием то, что система сама отвергла, нельзя: человек
    учит немецкий по нашему ответу. То же правило — в `кнопки_вариантов`.

    Правило одно и то же и здесь, и на применении решения: сервер берёт текст из базы
    (`database.personal_question_fix`), а не из браузера.
    """
    for судья in (судьи if isinstance(судьи, list) else []):
        if not isinstance(судья, dict):
            continue
        поле = str(судья.get("field") or "").strip()
        текст = str(судья.get("fix") or "").strip()
        плохой = str((судья.get("fix_check") or {}).get("state") or "") == "bad"
        if текст and поле in ("headword", "translation") and not плохой:
            return {"field": поле, "fix": текст}
    return {}
...
def audit_items(user_id: int, limit: int = 200) -> list[dict[str, Any]]:
    ...
    слова = [{"word": str(a), "translation": str(b), "status": str(c),
             "why": _human_reason(str(c), str(d), str(e)), "suggestion": str(e),
...
def _human_reason(status: str, source: str, suggestion: str = "") -> str:
    """Причина ЧЕЛОВЕЧЕСКИМ языком. Никаких «не подтверждено источником X».

    ┌─ ПОЧИНЕНО 04.09.2026. ПРИЧИНУ БОЛЬШЕ НЕ ДОДУМЫВАЕМ. ─────────────────────────────┐
    │ Здесь стояло одно на всех «Похоже, при сохранении слово потеряло часть букв» —    │
    │ на КАЖДЫЙ ответ модели «такого слова нет». Модель же говорит только «такого слова │
    │ нет»; про потерянные буквы мы додумывали сами. Владелец 04.09.2026 открыл экран и │
    │ увидел под этой надписью «aufknuspern» — слово, сохранённое 01.09 целиком, с      │
    │ переводом «разгрызть»: букв там не терялось, и рядом не стояло НИ ОДНОГО варианта.│
    │ «Ну а где было вот так — предлагаю вот так?»                                      │
    │                                                                                  │
    │ Теперь надпись зависит от того, есть ли у нас готовый вариант. Есть — говорим про │
    │ потерянные буквы и показываем целое слово кнопкой рядом (её рисует WordAudit.jsx  │
    │ по полю `suggestion`). Нет — говорим ровно то, что знаем, и человек вписывает своё.│
    │ Диагноз без готового варианта — брак: правило проекта «судья обязан предложить».  │
    └──────────────────────────────────────────────────────────────────────────────────┘
    """
    есть_вариант = bool(str(suggestion or "").strip())
    ...
    if source.startswith("модель: такого слова нет"):
        return ("Похоже, слово сохранилось не целиком. Вот целое — проверь:"
                if есть_вариант else
                "Справочники такого слова не знают, а восстановить написание мы не смогли.")
    if source.startswith("модель предложила другое"):
        return ("Написание спорное: справочники знают другое. Вот их вариант — проверь:"
                if есть_вариант else
                "Написание спорное: справочники знают это слово иначе, "
                "а какое имелось в виду — мы не определили.")
...
def _apply_phrase_decision(user_id: int, item: dict[str, Any]) -> str:
    ...
    if action == "fix_personal":
        # ⛔ ГОТОВЫЙ ВАРИАНТ БЕРЁТСЯ ИЗ БАЗЫ, А НЕ ИЗ БРАУЗЕРА.
        # Экран присылает только «нажал готовый вариант». Какой это был текст, сервер
        # читает сам: правка расходится по всей словарной статье и по карточкам других
        # людей, и принимать её текст с чужой стороны нельзя. Это то же правило, по
        # которому у кнопок вариантов сверяется сам текст, а не его номер (28.08.2026).
        from backend.database import personal_question_fix
        готовое = personal_question_fix(review_id)
        if not готовое:
            return ""
        if готовое["field"] == "headword":
            # Правка САМОЙ фразы — та же дверь, что у «своего варианта»: переименование
            # статьи с развозом по всем местам и пересборкой разбора.
            итог = apply_phrase_review_decision(review_id, "replace", готовое["fix"])
            return "исправлено" if itog_text(итог) else "оставлено"
        # Правка ПЕРЕВОДА — дверь правки карточки. Примеры отдаём те же, что лежат
        # сейчас: одно касание меняет перевод и только его.
        from backend.database import apply_panel_card_edit, phrase_review_card_examples
        with get_db_connection_context() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT card FROM bt_3_lex_units WHERE id = %s;",
                            (готовое["unit_id"],))
                строка = cur.fetchone()
        карточка = строка[0] if строка and isinstance(строка[0], dict) else {}
        итог = apply_panel_card_edit(
            review_id, translation=готовое["fix"],
            examples=phrase_review_card_examples(карточка))
        return "перевод исправлен" if итог.get("unit_id") else ""
