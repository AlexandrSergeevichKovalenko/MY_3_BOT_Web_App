...
_JUDGE_PROMPT = """\
Ниже немецкие слова для кроссворда. Все они уже проверены по словарю и по частоте:
слова существуют и встречаются в живой речи. Твоя задача узкая — поймать те, что
...
Сомневаешься — отвечай ДА. Слово уже прошло проверку словарём, а лишний отказ
обедняет тему: из-за строгой формулировки раньше вылетали MÜDIGKEIT и GESTRESST.
