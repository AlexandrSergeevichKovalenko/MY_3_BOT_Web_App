...
DICTIONARY_LLM_TASKS = (
    ...
    # Разбор для попапа выделения. Задача УБРАНА 30.08.2026 (попап переехал на общий
    # разбор словаря), но имя остаётся здесь: под ним лежат прошлые расходы в
    # ведомости, и без этой строки отчёт задним числом их потеряет.
