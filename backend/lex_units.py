...
# ┌─ ОДНО ПРАВИЛО «КАКОЙ РУССКИЙ ЧЕЛОВЕК ВИДИТ» — ЗДЕСЬ И БОЛЬШЕ НИГДЕ. ─────────────┐
# │ Перевод немецкой единицы лежит НЕ в её карточке, а отдельной русской единицей в  │
# │ слое связей. Какая из связей попадает на экран, решает порядок ниже: сначала     │
# │ выбор владельца, потом связь с разобранным значением, потом ранг.                │
# │                                                                                  │
# │ ЗАЧЕМ ВЫНЕСЕНО В КОНСТАНТУ (02.09.2026). У панели трёх голосов была своя копия   │
# │ ответа на этот вопрос — ключ `translation_ru` в сыром json карточки. Копия эта    │
# │ снималась при сохранении и с тех пор жила своей жизнью: замер по живой базе,     │
# │ 6738 карточек фраз — у 752 ключа нет вовсе, у 672 он разошёлся с тем, что на      │
# │ экране. Панель судила перевод, которого не видела: 890 вердиктов вынесены о       │
# │ пустом или чужом русском (546 — о пустом), из них 741 штамп «чисто» и 149         │
# │ вопросов живым людям («русский перевод не передаёт значение немецкой фразы» —     │
# │ при том что перевод в базе был). Владелец нашёл это на экране: «Перевода нет».    │
# │ Второй копии правила больше нет: и выдача, и все проверки спрашивают отсюда.      │
# └──────────────────────────────────────────────────────────────────────────────────┘
_LINK_PICK_WHERE = (f"l.rank < {_DEMOTED_RANK} "
                    "AND position('___' in v.display) = 0")
_LINK_PICK_ORDER = (f"(l.source IS DISTINCT FROM '{OWNER_CHOICE_SOURCE}' OR l.rank <> 1), "
                    "(l.sense_id IS NULL), l.rank, v.id")


def native_display_sql(unit_alias: str, lang: str = "ru") -> str:
    """Скалярный подзапрос: перевод, который человек ВИДИТ у единицы `<alias>.id`.

    Возвращает NULL, когда перевода нет вообще, — и это честное «нет», а не пустая
    строка, неотличимая от «мы не посмотрели». Проверки, которым перевод обязателен,
    обязаны отличать одно от другого.

    Алиас и язык подставляются в текст запроса, поэтому оба проверяются: сюда приходят
    только наши собственные буквы, но правило «не склеивать SQL из непроверенного»
    не имеет исключений «для своих».
    """
    if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", str(unit_alias or "")):
        raise ValueError(f"недопустимый алиас таблицы: {unit_alias!r}")
    if not re.fullmatch(r"[a-z]{2}", str(lang or "")):
        raise ValueError(f"недопустимый код языка: {lang!r}")
    return (f"(SELECT v.display FROM bt_3_lex_links l "
            f"   JOIN bt_3_lex_units v ON v.id = l.to_unit "
            f"  WHERE l.from_unit = {unit_alias}.id AND v.lang = '{lang}' "
            f"    AND {_LINK_PICK_WHERE} "
            f"  ORDER BY {_LINK_PICK_ORDER} LIMIT 1)")
...
def _fetch_links(cur, unit_id: int, *, want_lang: str) -> list[dict]:
    cur.execute(
        f"""
        SELECT v.id, v.lang, v.kind, v.display, v.lemma, v.pos, v.gender, l.rank, v.card,
               l.source
        ...
        --
        -- Отбор и порядок — ОДНОЙ строкой на всё приложение (`_LINK_PICK_*` выше).
        -- Проверки, которые судят перевод, спрашивают ровно это через
        -- `native_display_sql`, поэтому вторая копия правила здесь недопустима.
        WHERE l.from_unit = %s AND v.lang = %s
        ...
        (unit_id, want_lang, _MAX_LINKS * 4),
    )
    return [
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    WHERE u.lang = %s AND u.kind IN ('word', 'collocation') AND u.card IS NULL
                      -- ДВА ОТКАЗА ПО СУЩЕСТВУ — И НОЧЬ БОЛЬШЕ НЕ БЕРЁТ СЛОВО.
                      --
                      -- Доказано 29.08.2026: из 20 слов вчерашнего прогона у 4 разбор
                      -- не появился, и все 4 стояли в очереди снова — памяти об отказах
                      -- не было вовсе, слово бралось бесконечно по два платных запроса
                      -- за ночь.
                      --
                      -- Порог 2 и БЕЗ возврата через неделю — решение владельца
                      -- 29.08.2026: «если по какой-то причине оно не может быть
                      -- обработано, то в чём смысл ещё ждать и его запускать и тратить
                      -- деньги?». Судья отвечает при нулевой температуре, его вердикт
                      -- устойчив: третья попытка почти всегда даёт тот же ответ за те
                      -- же деньги.
                      --
                      -- Слово не пропадает и не удаляется молча: оно уходит владельцу
                      -- воскресным разбором с двумя кнопками — «оставить как есть»
                      -- (живёт в словаре без разбора, это не поломка) и «удалить».
                      -- Пока он не решил, ничего не происходит и денег не тратится.
                      --
                      ...
                      AND NOT EXISTS (
                          SELECT 1 FROM bt_3_unit_enrich_refusals f
                           WHERE f.unit_id = u.id AND f.refusals >= 2)
...
def attach_entry_to_unit(
    ...
    *,
    ...
    try:
        ...
        return None
    if card and lang == "de":
        # ⛔ ПРЕДЛОЖЕНИЮ РАЗБОР НЕ КЛАДЁМ. Решение владельца 27.08.2026: «это уже
        # предложение, включающее в себя контекст использования слов… главное — есть
        # немецкий и русский вариант, и больше ничего не нужно».
        #
        # Это была вторая дверь: разбор приезжал ВМЕСТЕ с сохранением (метка
        # «сохранение») и ложился на единицу-предложение как на слово. Замер
        # 27.08.2026 по живой базе: 28 предложений, из них 5 за один день.
        # Вид считаем тем же правилом, каким `ensure_unit` только что завёл единицу, —
        # правило одно на всё приложение и живёт в `_kind_for_text`.
        #
        # Связь с переводом ниже НЕ отключается: для предложения перевод и есть всё
        # его содержимое, и без связи ночная проверка грамматики его не увидит
        # (см. backend/translation_links.py).
        if _kind_for_text(text) != "sentence":
            try:
...
def card_link_values(card: dict, *, kind: str = "word") -> list[dict]:
    """Что из разбора идёт в связи слова и в каком ПОРЯДКЕ.

    Отдельной функцией — чтобы порядок можно было проверить тестом без базы:
    первым в списке стоит то, что человек увидит строкой «ПЕРЕВОД».
    Возвращает [{"value": …, "note": …}] без повторов; пустой список — если в
    разборе переводов нет вовсе.

    `kind` — вид единицы из базы. У СЛОВА переводы идут раньше значений, у ФРАЗЫ
    наоборот: это измерено на живых данных, замер в комментарии ниже.
    """
        ...
        for item in items:
            for part in split_translation(item["value"]):
                ...
                if not key:
                    continue
                if key in seen:
                    # Одно и то же слово лежит и в переводах, и в значениях. Берём его
                    # один раз, но пояснение из значения не выбрасываем: у перевода
                    # пояснения нет, а у значения оно и есть весь контекст.
                    if note:
                        for existing in unique:
                            if normalize_query(existing["value"]) == key:
                                existing["note"] = "; ".join(
                                    x for x in (existing.get("note"), note) if x)[:500]
                                break
                    continue
                seen.add(key)
    ...
    # ┌─ ПРОВЕРЕНО 31.08.2026. ПЕРЕВОД БЕРЁМ ИЗ ПОЛЯ ПЕРЕВОДОВ. НЕ ПОДНИМАТЬ ЗАНОВО. ──┐
    # │ Первой связью (ранг 10) встаёт то, что человек увидит строкой «ПЕРЕВОД» в       │
    # │ быстром словаре. Раньше туда шло `meanings.primary.value`, а список переводов   │
    # │ смотрели, только если из значений ничего не осталось. Но у промпта это РАЗНЫЕ   │
    # │ поля: `translations[]` — эквиваленты, `meanings.*.value` — значение, и туда     │
    # │ законно попадает ТОЛКОВАНИЕ. Так «Verschwörer» показывал «человек, участвующий  │
    # │ в заговоре», хотя рядом в той же карточке лежало «заговорщик», и именно его     │
    # │ карточка печатает ниже в «ВАРИАНТАХ ПЕРЕВОДА» (владелец увидел это 31.08.2026). │
    # │                                                                                 │
    # │ ЧТО МЕРИЛИ И ЧТО ОТВЕРГЛИ — чтобы не мерить второй раз:                         │
    # │ • Признак «это толкование» ПО ВИДУ СТРОКИ отвергнут: «5+ слов» ловит 84 случая  │
    # │   вместе с законными перечислениями («немедленно, сразу же, без промедления»),  │
    # │   «запятая + причастие» — 29, и почти все они законные пары «обычный,           │
    # │   традиционный». Гадать по форме нельзя — поле у нас есть.                      │
    # │ • Управляемый замер (одна карточка, старый порядок против нового) на 3 263      │
    # │   одиночных словах: первый перевод меняется у 213, у 2 925 остаётся прежним.    │
    # │ • МАССОВУЮ перестановку уже накопленных связей делать НЕЛЬЗЯ, это проверено:    │
    # │   прогон против того, что лежит в базе, двигает 2 083 из 5 389 — но сегодняшняя │
    # │   первая связь часто пришла из живого сохранения или пула и бывает ЛУЧШЕ        │
    # │   карточки («abstellen: отключать → ставить», «schwerfällig: неуклюжий →        │
    # │   тяжелый»). Поэтому старое не трогали: по строгому признаку «родовое слово +   │
    # │   придаточное» нашлось 21 кандидат на всю базу, разобраны глазами, перевешены   │
    # │   8 настоящих (Verschwörer, Flaute, Auswanderung, Ausrüstung, Phänomen,         │
    # │   Bescheinigung, Erbstück, Evolutionsbremse).                                   │
    # │ Перемерить: scripts/lex_links_translation_first.py --dry-run                    │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    # Порядок склеен, а не выбран «или-или»: у «das Musterkind» все значения —
    # определения на сто с лишним знаков, а рядом лежат готовые «примерный ребёнок»,
    # «идеальный ребёнок». Раньше до них не доходили и слово оставалось вовсе без
    # перевода; теперь оба списка идут в одну очередь, переводы первыми.
    # ФРАЗЕ — НАОБОРОТ, И ЭТО ИЗМЕРЕНО, А НЕ ПРЕДПОЛОЖЕНО (31.08.2026).
    # Прогон по 100 живым единицам показал: у оборотов и предложений `translations[]`
    # держит не перевод, а обрывок или пример — «in der Dämmerung» получал «сумерки»
    # вместо «в сумерках», «das Verfahren einstellen» — «Адвокат подал ходатайство о
    # прекращении дела», «den Umfang der Projekte bestimmen» — «масштаб проектов».
    # Заслон `looks_like_example_not_translation` тут не спасает: он намеренно работает
    # только для слов (у фразы перевод целым предложением — норма). Поэтому у фразы
    # порядок прежний: значение первым.
    if str(kind or "") != "word":
        return _pick(values + from_translations)
    return _pick(from_translations + values)
...
def sync_unit_links_from_card(unit_id: int, card: dict, *, native_lang: str = "ru") -> dict:
    ...
    Поэтому после появления разбора переводы берём из него: сперва то, что разбор
    назвал ПЕРЕВОДОМ, следом — значения (порядок собирает `card_link_values`). Старые
    связи НЕ удаляем — просто отодвигаем ниже: они могли прийти из живого сохранения
    человека, и терять их нельзя."""
    if not isinstance(card, dict) or not card:
    ...
    try:
        with get_db_connection_context() as conn:
            with conn.cursor() as cur:
                ...
                # Порядок переводов зависит от вида единицы, поэтому список собираем
                # ЗДЕСЬ: выше по коду вид ещё не прочитан из базы.
                unique = card_link_values(card, kind=kind_of_source)
                if not unique:
                    return {"senses": 0, "links": 0}
                ...
                # ОБЕ СТОРОНЫ. Связь пишется двумя строками (de→ru и ru→de), а выдача
                # сортирует по обеим: `entries_for_query` берёт минимальный ранг без
                # оглядки на то, с какой стороны он записан. Пока отодвигали только
                # `from_unit`, обратная строка оставалась с прежним рангом и всё равно
                # вставала первой — у «Auswanderung» так и висело «Постоянный переезд из
                # родной страны в другую страну» (ранг 10 со стороны ru), хотя со стороны
                # de первой уже стояла «эмиграция» (проверено 31.08.2026).
                cur.execute(
                    "UPDATE bt_3_lex_links SET rank = GREATEST(rank, 30) "
                    "WHERE (from_unit = %s OR to_unit = %s) AND rank < 30;",
                    (unit_id, unit_id),
                )
