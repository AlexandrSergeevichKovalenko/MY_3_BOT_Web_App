...
FIXES_ITSELF = "сама"
NEEDS_OWNER = "кнопка"
# ┌─ ПОЧИНЕНО 29.08.2026. МОЯ РАБОТА ПРИХОДИЛА ВЛАДЕЛЬЦУ КАК ЕГО. ────────────────┐
# │ Куч было ДВЕ: «чинится само» и «нужен человек». Третьей — «это задача         │
# │ разработчика» — не существовало, поэтому правило «немецкий текст пишется мимо │
# │ двери» лежало в NEEDS_OWNER, хотя в его же пояснении написано «нужен я, а не  │
# │ владелец». Итог: строка дня говорила «📖 Словарь: 1 записей машина закрыть не │
# │ может — нужен ты», а делать владельцу было НЕЧЕГО, и кнопки под этим не было  │
# │ и быть не могло. Владелец 29.08.2026: «здесь ни одной кнопки нету, здесь      │
# │ нужна какая-то моя активность или нет? если нет — зачем это мне нужно?»       │
# │                                                                              │
# │ Правило: «нужен ты» пишется ТОЛЬКО там, где владельцу есть куда нажать. Всё,  │
# │ что чиню я, считается отдельно и его не зовёт. Перемерить: owner_line() при   │
# │ единственном ненулевом правиле NEEDS_ME не должна содержать слов «нужен ты».  │
# └──────────────────────────────────────────────────────────────────────────────┘
NEEDS_ME = "моя задача"
...
CLOSED_BY: dict[str, tuple[str, str]] = {
    ...
    "немецкий текст пишется мимо двери": (NEEDS_ME, "новый путь записи мимо двери — "
                                                    "закрываю я, владельцу делать нечего"),
...
def owner_line() -> str:
    ...
    людям, машине, моих = 0, 0, 0
    for title, count in result["broken"].items():
        if not count:
            continue
        # Незнакомое правило считаем СВОЕЙ задачей, а не владельца: если я завёл
        # правило и забыл сказать, кто его закрывает, звать по этому поводу владельца
        # нечестно — кнопки под ним всё равно нет.
        кто, _ = CLOSED_BY.get(title, (NEEDS_ME, ""))
        if кто == NEEDS_OWNER:
            людям += count
        elif кто == NEEDS_ME:
            моих += count
    ...
    # «НУЖЕН ТЫ» — ТОЛЬКО ТАМ, ГДЕ ЕСТЬ КУДА НАЖАТЬ. Мои задачи владельца не зовут:
    # он всё равно ничего с ними не сделает, а строка отнимет внимание (см. врезку
    # у NEEDS_ME).
    if людям:
        return (f"📖 Словарь: <b>{людям}</b> записей машина закрыть не может — нужен ты. "
                f"Разбор: /admin_dict_integrity")
    if машине or моих:
        хвост = f" Ещё {моих} — моя задача, не твоя." if моих else ""
        видимых = машине or моих
        return (f"📖 Словарь в порядке. {видимых} записей разбираются под капотом — "
                f"от тебя ничего не нужно.{хвост}")
...
def report_lines() -> list[str]:
    ...
    for title, count in sorted(result["broken"].items(), key=lambda kv: -kv[1]):
        if not count:
            continue
        who, how = CLOSED_BY.get(title, (NEEDS_ME, "закрывать нечем — это моя задача"))
        строка = f"  • {title}: {count} — {how}"
        if who == NEEDS_OWNER:
            твоё.append(строка)
        elif who == NEEDS_ME:
            моё.append(строка)
    ...
    # ТРИ КУЧИ, А НЕ ДВЕ. Раньше их было две, и мои задачи попадали под заголовок
    # «Нужен человек» — владелец читал его как «нужен ты» и искал кнопку, которой нет.
    lines = [f"Целостность словаря: {result['total']} нарушений"]
    if not твоё:
        lines.append("От тебя сейчас ничего не нужно — нажимать нечего.")
    if ночью:
        lines.append("Чинится само ночью, смотреть не надо (число должно убывать):")
        lines.extend(ночью)
    if моё:
        lines.append("Моя работа, не твоя (правка в коде — делаю я):")
        lines.extend(моё)
    if твоё:
        lines.append("НУЖЕН ТЫ — здесь есть что нажать:")
        lines.extend(твоё)
