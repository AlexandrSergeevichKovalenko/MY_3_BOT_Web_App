...
def test_хозяин_бывает_только_трёх_видов():
    """29.08.2026 куч стало ТРИ, а не две.

    Было «чинится само» и «нужен человек», и всё, что не чинилось само, падало
    владельцу — включая правки в коде. Он получил строку «нужен ты» на задачу, под
    которой нет и не может быть кнопки, и справедливо спросил: «здесь нужна какая-то
    моя активность или нет? если нет — зачем это мне?». Появилась третья куча NEEDS_ME.

    Требование прежнее и не ослаблено: у каждого правила назван ХОЗЯИН и сказано, ЧЕМ
    оно закрывается. Добавилось лишь то, что хозяев теперь трое.
    """
    for title, (who, how) in integrity.CLOSED_BY.items():
        assert who in (integrity.FIXES_ITSELF, integrity.NEEDS_OWNER,
                       integrity.NEEDS_ME), title
...
def test_самочинящееся_отделено_от_моей_работы(monkeypatch):
    """Разделение осталось, но границы стало ТРИ, и «мимо двери» — моя работа.

    Раньше эти два правила ложились в кучи «само» и «нужен человек», и владельца звали
    на второе. Теперь второе честно названо моей задачей, а заголовок «НУЖЕН ТЫ»
    появляется только когда владельцу есть куда нажать (см.
    test_integrity_never_calls_the_owner_for_my_work).
    """
    lines = _report(monkeypatch, {"глагол без источника спряжения": 33,
                                  "немецкий текст пишется мимо двери": 2})
    text = "\n".join(lines)
    assert "Чинится само ночью" in text
    assert "Моя работа, не твоя" in text
    assert "НУЖЕН ТЫ" not in text, "владельца зовут туда, где нажимать нечего"
    assert "От тебя сейчас ничего не нужно" in text
