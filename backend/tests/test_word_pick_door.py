...
class ДверьОтбора(unittest.TestCase):
    ...
    def test_все_источники_тапа_разрешены_и_не_превращаются_в_unknown(self):
        ...
        self.assertEqual(db.WORD_PICK_ORIGINS, frozenset({
            ...
            "trainer_text_save",
            # 13.09.2026: «Подставь синоним» — среда рельса. Две СВОИ поверхности:
            # чип в итоге захода (слово, которое человек вписал сам) и тап по слову
            # в немецком предложении с пропуском. Вписанное своей рукой просится на
            # завтрашнее повторение не слабее тапнутого в тренажёре.
