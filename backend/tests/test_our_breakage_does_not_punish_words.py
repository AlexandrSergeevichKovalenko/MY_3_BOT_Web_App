...
class ОчередьПропускаетОтложенные(unittest.TestCase):
    def test_the_picker_skips_paused_words(self):
        ...
        # Порог и судьба слова изменены владельцем в тот же день, 29.08.2026: два
        # отказа вместо трёх, и БЕЗ возврата через неделю — вместо круга слово идёт
        # к нему с кнопками. Подробности и остальные обещания —
        # test_words_go_to_owner_not_in_circles.py.
        self.assertIn("refusals >= 2", тело)
