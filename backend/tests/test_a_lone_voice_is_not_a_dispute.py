...
class ОдинокаяПретензияНеДоезжаетДоЧеловекаTests(unittest.TestCase):
    ...
    def test_one_claim_and_one_silence_out_of_two_is_not_a_dispute(self):
        """Именно так родились 45 из 51 вопроса на экране владельца."""
        from backend import phrase_panel as pp
        self.assertEqual(self._вердикт([{"headword"}, set(), None]), pp.NOT_ASKED)
...
class ПанельНеРаботаетНаДвухГолосахМолчаTests(unittest.TestCase):
    ...
    def test_the_night_does_not_start_on_two_voices(self):
        from backend import phrase_panel as pp
        ...
        self.assertIn("429", отчёт["пропущено"])
