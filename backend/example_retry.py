...
def retry_batch(*, limit: int = 20, budget_usd: float = 0.30) -> dict[str, Any]:
    ...
    with get_db_connection_context() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                LIMIT %s;""", (FIELD, DEFECT, int(limit)))
            rows = cur.fetchall()
    ...
    for unit_id, display, kind, card, why, attempts in rows:
        if spent >= budget_usd:
        ...
        ask = {"entry": display, "kind": kind,
               ...
               "previous_attempt_was_rejected_because": str(why or "")[:300]}
        try:
            ...
            with conn.cursor() as cur:
                if ok and lex_units.save_unit_card(int(unit_id), candidate, source=SOURCE):
                    cur.execute("""UPDATE bt_3_field_checks
                                   ...
                                   WHERE unit_id=%s AND field=%s;""",
                                (CLEAN, "примеры переписаны ночным повтором", unit_id, FIELD))
                    report["переписано"] += 1
