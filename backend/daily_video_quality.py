...
# ┌─ СНЯТО 05.09.2026, РЕШЕНИЕ ВЛАДЕЛЬЦА: «цитата дословно в субтитрах» больше не бракует. ─┐
# │ Здесь стояла проверка «цитата выдумана»: цитата обязана буква в букву найтись в         │
# │ субтитрах. Это была сверка строк, а не оценка смысла, и в ночь на 05.09 она сняла все   │
# │ 8 карточек новостей за 04.09 разом. Владелец: «если модель чуть переставила слово, а    │
# │ смысл тот же, выбрасывать верное выражение с верным переводом глупо — карточку          │
# │ оставляем». Цитату модель по-прежнему берёт из субтитров (шаг «извлечь»), недословные   │
# │ считаются и пишутся в лог генератора (not_verbatim), но карточку не губят.               │
# └─────────────────────────────────────────────────────────────────────────────────────────┘
...
_CHECKS_NEEDING_TRANSCRIPT = {"форма из текста выдумана"}
