...
def fetch_transcript_or_verdict(video_id: str, *, timeout_sec: int = 150,
    ...
    отдаёт субтитры за 1–7 секунд, полный отказ лестницы занимает 91–95 секунд.
    Поэтому таймаут по умолчанию ВЫШЕ этой длины: он обязан ловить зависание, а не
    обрывать лестницу за секунду до внятного ответа (проверено живым прогоном 29.08.2026 —
    с 90 секундами вердикт всегда выходил «не дождались», и приговор не выносился никогда).
