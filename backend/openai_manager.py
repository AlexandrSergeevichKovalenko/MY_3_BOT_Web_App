...
ЧТО ТЕБЕ ДАЮТ. По каждому слову — наша словарная статья:
{"word":"ausweisen","pos":"verb","senses":[{"meaning":"выдворять","context":"о решении государства"}],
 "examples":[{"de":"...","ru":"..."}],"constructions":[{"pattern":"jdn. ausweisen","case":"Akkusativ"}],
 "collocations":["sich ausweisen"],"register":""}

ГЛАВНОЕ ПРАВИЛО. Сравнивать разрешено ТОЛЬКО поданные статьи. Значения, сферы, примеры и
сочетания, которых в них нет, придумывать ЗАПРЕЩЕНО. Не хватает данных на поле — оставь
поле пустым. Пустое поле честно, выдуманное — брак. Это учебное приложение: человек
заучит то, что ты напишешь.

ЧЕГО НЕ ВОЗВРАЩАТЬ ВООБЩЕ: род, артикль, множественное число, падежи, часть речи,
управление, уровень слова. Эти данные берутся из справочника, твой вариант отбрасывается.
Не пиши этимологию, историю слова, списки синонимов и антонимов, частотность, рассуждения
об уровне и повторы одной мысли разными словами.

СНАЧАЛА РЕШИ, СРАВНИМЫ ЛИ СЛОВА. Человек мог свести слова, которые синонимами не являются.
Смотри на поданные значения и определи:
  "overlap"   — слова правда пересекаются по смыслу;
  "one_sense" — пересекается ТОЛЬКО одно значение (у ausweisen — «выдворить», а не
                «удостоверить личность»); сравнивай ИМЕННО его и назови его вслух;
  "none"      — общего смысла нет, слова лишь похоже переводятся на язык человека.
Не подгоняй слова под сходство и не бери побочное значение, если основное к сравнению ближе.

БЛОКИ:
- comparable: {"value":"overlap|one_sense|none","note":"одной фразой, какое значение сравниваем"}.
- verdict: по строке на КАЖДОЕ слово, 8-14 слов, простым языком. Это первое, что читают.
- overlap: {"note":"где именно значения пересекаются","roles":[{"word":"...","role":"чем оно
  в этом пересечении является"}]} — например ausweisen → решение, abschieben → исполнение.
  Пересечения нет — note объясняет это, roles пустой.
- interchangeable: {"value":"no|sometimes|yes","note":"..."}; note обязателен всегда.
- words: по записи на каждое слово: meaning — значение ИМЕННО в этом сравнении (не весь
  словарь), when — когда берут это слово, а не соседнее, register — только если разница
  регистра действительно есть (юридическое, разговорное, грубое, устаревшее), иначе "".
- examples: СИММЕТРИЯ ОБЯЗАТЕЛЬНА. Для КАЖДОГО слова — свой верный пример (correct: true).
  К каждому верному добавь противопоставление на ТОЙ ЖЕ ситуации (correct: false): то же
  предложение с соседним словом и почему так не говорят. У неверного в поле word стоит
  слово, которое поставили ошибочно. Ни одно слово не должно остаться без верного примера.
- chooser: по строке на слово — короткое правило выбора, чтобы человек выбрал слово по нему.
- trap: настоящая ошибка человека, у которого родной язык explain_language. Нет такой —
  пустая строка, придумывать не надо.
- collocations: до ТРЁХ на КАЖДОЕ слово, с переводом, ТОЛЬКО из поданных сочетаний,
  конструкций и примеров. Ни одно слово не должно остаться без сочетаний, если они поданы.

Объяснения (line, note, role, meaning, when, why, situation, trap, translation) — на
explain_language. Немецкие поля (de, phrase) — на studied_language.

Верни СТРОГО ОДИН JSON без текста вокруг:
{
  "comparable": {"value":"...","note":"..."},
  "verdict": [{"word":"...","line":"..."}],
  "overlap": {"note":"...","roles":[{"word":"...","role":"..."}]},
  "interchangeable": {"value":"...","note":"..."},
  "words": [{"word":"...","meaning":"...","when":"...","register":""}],
  "examples": [{"word":"...","correct":true,"de":"...","translation":"...","why":""}],
...
ОТЛИЧИЕ ЭТОГО РЕЖИМА: ты возвращаешь НЕ один объект, а НЕСКОЛЬКО подряд, по одному на блок,
в этом порядке. Между объектами не пиши ничего — ни запятых, ни текста, ни markdown.

1) {"section":"comparable","comparable":{"value":"...","note":"..."}}
2) {"section":"verdict","verdict":[{"word":"...","line":"..."}]}
3) {"section":"overlap","overlap":{"note":"...","roles":[{"word":"...","role":"..."}]}}
4) {"section":"interchangeable","interchangeable":{"value":"...","note":"..."}}
5) {"section":"words","words":[{"word":"...","meaning":"...","when":"...","register":""}]}
6) {"section":"examples","examples":[{"word":"...","correct":true,"de":"...","translation":"...","why":""}]}
7) {"section":"chooser","chooser":[{"situation":"...","word":"..."}]}
8) {"section":"trap","trap":"..."}
9) {"section":"collocations","collocations":[{"word":"...","phrase":"...","translation":"..."}]}

Первые два объекта — самые важные: человек читает их, пока идут остальные. Не откладывай их.
""",
"quiz_followup_question": """
