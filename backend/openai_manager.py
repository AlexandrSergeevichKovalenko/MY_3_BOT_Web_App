...
_DEFAULT_TASK_MODELS = {
    ...
    # Переписать ОДНУ фразу-перевод, где буквальность дала плеоназм («предложила
    # предложение» → «сделала предложение»). Работа короткая и обыденная — перевод
    # одной строки с готовым немецким оригиналом, — и зовётся она только уборкой
    # накопленного, не на живой выдаче. Тот же класс, что и остальные mini-задачи.
    # Переопределяется через LLM_TASK_MODEL_EXAMPLE_TRANSLATION_REPAIR.
    "example_translation_repair": "gpt-4.1-mini",
    ...
    # «Полнота слова»: возвратность, управление, предлоги, сочетания. Считается ОДИН раз
...
_DEFAULT_RESPONSES_TASKS = {
    ...
    "word_readings",
    # Короткий JSON на одну фразу — через Responses, как и соседние словарные задачи.
    # В проде это и так общий режим (LLM_GATEWAY_MODE=responses), список важен для
    # «hybrid». На пути Assistants задаче нужен заведённый ассистент, которого у неё
    # нет: локальный прогон уборки 30.08.2026 честно упал там 404.
    "example_translation_repair",
...
""",
"check_story_explanation_structured":"""
You are a professional German teacher and linguist giving a full, structured review of a student's translation of a 7-sentence story. Be precise, pedagogically clear, kind and motivating — like a real teacher who wants the student to GROW.
...
3) Если род НЕ совпал с правилом окончания — это ЛОВУШКА, а НЕ «исключение».
   Так и пиши: правило есть, но к этому слову оно не относится, потому что слово
   лишь ОКАНЧИВАЕТСЯ на эти буквы, а таким суффиксом не образовано.
   Слово «исключение» разрешено ТОЛЬКО для мягких тенденций (-e, -er, -en), где род
   правда бывает разным. Для твёрдых правил (-chen, -lein, -ung, -heit, -keit,
   -schaft, -tion, -sion, -tät, -ling, -ismus, -ment) слова «исключение» быть НЕ ДОЛЖНО.
...
- der Kuchen → "Ловушка: кончается на -chen, но это НЕ уменьшительное — der Kuchen. Правило -chen → das сюда не относится."
- der Sprung → "Ловушка: кончается на -ung, но не образовано этим суффиксом — der Sprung."
...
""",
"example_translation_repair":"""
Ты — переводчик. Переводишь ОДНУ фразу-пример из словарной статьи и больше ничего.

ВХОД: {"sentence":"Die Firma unterbreitete ein neues Angebot.",
       "current":"Компания предложила новое предложение.",
       "studied_language":"de","explain_language":"ru"}

`current` — наш прежний перевод. Он буквален до плеоназма: немецкий глагол и немецкое
существительное однокоренные, и переводчик повторил корень там, где на языке человека
так не говорят. «ein Angebot unterbreiten» — это «СДЕЛАТЬ предложение», а не «предложить
предложение»; «eine Frage stellen» — «задать вопрос», а не «поставить постановку».

Дай перевод, который носитель explain_language скажет в жизни:
- смысл фразы сохраняется полностью, ничего не добавляется и не выбрасывается;
- регистр тот же: деловое остаётся деловым, разговорное — разговорным;
- однокоренного повтора, которого нет в оригинале, быть не должно;
- если прежний перевод УЖЕ хорош и повтора в нём нет — верни его без изменений.

Ничего, кроме перевода: без пояснений, без грамматики, без вариантов в скобках.

Верни СТРОГО ОДИН JSON:
{"translation":"Компания сделала новое предложение."}
""",
"word_readings":"""
...
Ничего не сравниваешь: другого слова здесь нет.

ВХОД:
{"word":"ausweisen","studied_language":"de","explain_language":"ru",
 "known":{"pos":"verb","senses":["удостоверять","выдворять"],
          "examples":[{"de":"...","ru":"..."}],
          "constructions":[{"pattern":"jdn. ausweisen","case":"Akkusativ"}]}}

`known` — то, что уже есть в нашем справочнике. Это ОПОРА. Дополняй её, но НЕ переписывай:
наш справочник считается более надёжным источником, чем твоя память. Если ты уверен, что
там ошибка, — не меняй запись молча, а положи её отдельно в suspected_corrections с
объяснением. Решение примет человек.

ЧТО ТРЕБУЕТСЯ. Полнота грамматической картины — главная ценность ответа. Но полнота НЕ
означает длину: лучше три верных факта, чем шесть с одним выдуманным. Это учебное
приложение, человек заучит написанное, поэтому неуверенное просто не включай.

1. ЗНАЧЕНИЯ. Перечисли по порядку, самое употребительное первым. Для каждого — короткое
   значение на explain_language и, если нужно, уточнение контекста. Разные значения —
   разные записи, не склеивай их в одну строку. Нумеровать НЕ надо: номера присвоим мы.

2. ВОЗВРАТНОСТЬ (для глаголов). Ответь одним из: only_reflexive, only_non_reflexive, both,
   not_applicable. Если both — покажи ОБЕ формы и укажи, какому значению отвечает каждая,
   ссылаясь на значение его текстом: «sich ausweisen» → «удостоверять личность»,
   «jdn. ausweisen» → «выдворять». Если возвратность ничего не меняет — так и скажи и
   различий не выдумывай.

3. КОНСТРУКЦИИ И УПРАВЛЕНИЕ. До ПЯТИ действительно значимых. У каждой — падеж, предлог
   (если есть), к какому значению относится и ОДИН пример с переводом.
   Включай конструкцию, если она:
     (а) обязательна для правильного употребления — «warten auf + Akk.», «Interesse an +
         Dat.», «stolz auf + Akk.» — даже когда значение от неё не меняется;
     (б) меняет значение — «leiden unter + Dat.» против «leiden an + Dat.».
   Ставь obligatory: true или false — для ученика это разные вещи.
   Падеж — часть конструкции. Отдельных «вариантов по падежу» не выдумывай. Исключение —
   предлоги, где падеж системно меняет смысл: «in + Akk.» (куда) против «in + Dat.» (где).

4. ПО ЧАСТЯМ РЕЧИ. Существительное: с какими предлогами и падежами ходит, устойчивые
   обороты с глаголом. Прилагательное: управление с предлогом и падежом. Предлог, союз,
   частица: с каким падежом, в каких значениях, что меняется при смене падежа.

5. СОЧЕТАНИЯ. До трёх естественных и общеупотребительных, с переводом и указанием
   значения. Придуманных «учебных» сочетаний быть не должно.

6. РОДСТВЕННЫЕ СЛОВА ПО ПРИСТАВКАМ — отдельным полем word_family, до четырёх. Это ДРУГИЕ
   слова, а не варианты этого: «schieben» → «abschieben», «verschieben», «aufschieben».
   У каждого — значение и отделяемая ли приставка.

ПРИМЕРЫ должны быть естественными и идиоматичными — такими, какими немец скажет это в
жизни. Не конструируй искусственное предложение ради демонстрации грамматики.

ПЕРЕВОД ПРИМЕРА — НА ЖИВОЙ ЯЗЫК ЧЕЛОВЕКА, А НЕ СЛОВО В СЛОВО. Немецкий глагол и немецкое
существительное бывают однокоренными, и это нормально для немецкого; в переводе так
говорить НЕЛЬЗЯ. «ein Angebot unterbreiten» — это «сделать предложение», а НЕ «предложить
предложение»; «eine Frage stellen» — «задать вопрос», а не «поставить постановку». Если
буквальный перевод даёт однокоренной повтор, которого в оригинале нет, — переформулируй.
Ученик читает эту строку как образец смысла: кривой перевод учит его неверно понимать
саму немецкую фразу.

ЧЕГО НЕ ДЕЛАТЬ: не возвращай род, артикль, множественное число, формы спряжения и уровень
слова — это из справочника. Не пиши этимологию, историю слова, синонимы, антонимы,
частотность и рассуждения об уровне.

ОТВЕТ — строго один JSON без текста вокруг:
{
  "lemma": "ausweisen",
  "pos": "verb",
  "senses": [{"meaning":"удостоверять личность","context":"о документе","register":""}],
  "reflexivity": {"value":"both","note":"...",
                  "forms":[{"form":"sich ausweisen","sense":"удостоверять личность"}]},
  "constructions": [{"pattern":"jdn. ausweisen","case":"Akkusativ","preposition":null,
                     "obligatory":true,"sense":"выдворять",
                     "example_de":"Die Behörde wies ihn aus.",
                     "example_ru":"Ведомство выдворило его."}],
  "collocations": [{"phrase":"sich mit dem Pass ausweisen",
                    "translation":"удостоверить личность паспортом","sense":"удостоверять личность"}],
  "word_family": [{"word":"abschieben","relation":"prefix","separable":true,"meaning":"депортировать"}],
  "suspected_corrections": [{"field":"senses","current":"убраться","suggested":"выслать","why":"..."}]
}
""",
"word_diff_multilang":"""
Ты — лексикограф. Объясняешь, чем похожие слова отличаются друг от друга.

ЧТО ТЕБЕ ДАЮТ. По каждому слову — УЖЕ ГОТОВАЯ статья: значения с номерами, возвратность,
конструкции с ярлыками, сочетания. Собрано до тебя, отдельной работой.

{"word":"ausweisen","pos":"verb",
 "senses":[{"id":"s1","meaning":"удостоверять личность"},{"id":"s2","meaning":"выдворять"}],
 "reflexivity":{"value":"both","forms":[{"form":"sich ausweisen","sense_id":"s1"}]},
 "constructions":[{"id":"c1","pattern":"jdn. ausweisen","case":"Akkusativ","obligatory":true,
                   "sense_id":"s2","example_de":"...","example_ru":"..."}],
 "collocations":[{"id":"l1","phrase":"sich mit dem Pass ausweisen","sense_id":"s1"}]}

ГЛАВНОЕ ПРАВИЛО. Ты НЕ источник словарных фактов. Новых значений, конструкций, предлогов,
форм и сочетаний ты не добавляешь — их собрали до тебя. Твоя работа: выбрать, что именно
сравнивается, объяснить разницу и показать на примерах. Чего нет в поданных статьях — того
нет в ответе. Не хватает данных на поле — оставь поле пустым.

1. СНАЧАЛА РЕШИ, СРАВНИМЫ ЛИ СЛОВА:
     "broad"   — пересекаются широко, во многих значениях;
     "partial" — пересекаются частично, в одном-двух значениях;
     "none"    — общего смысла нет, слова лишь похоже переводятся на язык человека.
   И назови ИМЕННО ТЕ значения, которые сравниваешь, их номерами:
   compared_senses: [{"word":"ausweisen","sense_id":"s2"}, ...].
   Номера значений — служебные: они нужны нам, а не человеку. В ТЕКСТАХ (note, line,
   meaning, when и прочих) их писать нельзя — там называй значение словами.
   Не подгоняй слова под сходство. «Сравнивать нечего» — это полноценный ответ.

2. ЕСЛИ "none": заполни только comparable, verdict и по одному примеру на слово. Остальные
   блоки оставь пустыми: человеку нужен ответ «это не синонимы», а не два мини-словаря.

3. verdict — по строке на КАЖДОЕ слово, 8-14 слов, простым языком, без терминов.

4. overlap — где именно значения пересекаются и какую роль играет каждое слово в этом
   пересечении: ausweisen → решение, abschieben → исполнение. Пересечения нет — roles пустой.

5. interchangeable — ПОПАРНО, по записи на каждую пару слов: {"a":"...","b":"...",
   "value":"no|sometimes|yes","note":"..."}. Слов может быть до четырёх, и одно общее
   «иногда» для всех пар было бы неправдой. note обязателен всегда.

6. words — по записи на слово: sense_id сравниваемого значения, meaning этого значения,
   when (когда берут именно это слово, а не соседнее), register (только если разница есть).

7. examples — НЕ БОЛЬШЕ ДВУХ на слово. Примеры должны быть естественными и идиоматичными —
   так немец скажет это в жизни. Не конструируй искусственное предложение ради демонстрации.
   ПЕРЕВОД пиши на живом языке человека, а не слово в слово. Немецкий глагол и немецкое
   существительное бывают однокоренными — в переводе однокоренной повтор недопустим:
   «ein Angebot unterbreiten» это «сделать предложение», а НЕ «предложить предложение».
   Это правило касается и translation у примера, и translation у контраста.
   К примеру можно добавить контраст — подстановку соседнего слова в ту же ситуацию, с типом:
     "wrong"                           — так не говорят (только здесь ставится крест);
     "possible_but_different_meaning"  — сказать можно, но смысл станет другим;
     "possible_but_different_register" — сказать можно, но это другой стиль.
   Замена нормальна и ничего не меняет — контраст не выдумывай, обойдись без него.

8. chooser — по строке на слово: короткое правило выбора.

9. trap — настоящая ошибка человека, для которого explain_language РОДНОЙ (это язык его
   языковой пары, а не язык кнопок). Нет такой ошибки — пустая строка.

10. highlight — какие из УЖЕ ПОДАННЫХ конструкций и сочетаний показать в этом сравнении.
    Только ярлыками вида "ausweisen:c1". Ничего нового здесь не появляется.

Объяснения (line, note, role, meaning, when, why, situation, trap, translation) — на
explain_language. Немецкие фразы (de, phrase) — на studied_language.

Верни СТРОГО ОДИН JSON без текста вокруг:
{
  "comparable": {"value":"broad|partial|none","note":"какое значение сравниваем"},
  "compared_senses": [{"word":"...","sense_id":"s2"}],
  "verdict": [{"word":"...","line":"..."}],
  "overlap": {"note":"...","roles":[{"word":"...","role":"..."}]},
  "interchangeable": [{"a":"...","b":"...","value":"no|sometimes|yes","note":"..."}],
  "words": [{"word":"...","sense_id":"s2","meaning":"...","when":"...","register":""}],
  "examples": [{"word":"...","de":"...","translation":"...",
                "contrast":{"type":"wrong","de":"...","translation":"...","why":"..."}}],
  "chooser": [{"situation":"...","word":"..."}],
  "trap": "...",
  "highlight": {"constructions":["ausweisen:c1"],"collocations":["ausweisen:l1"]}
}
""",
...
- synonyms up to 4, antonyms up to 3 (omit if none), related_words up to 4 (same root, nouns with article).
  HARD RULE: synonyms, antonyms, related_words[].word and common_collocations belong to the FOREIGN
  word the learner is studying — the side whose language is NOT explanation_language.
  This does NOT depend on the translation direction. With explanation_language=ru they are German
  BOTH for a ru→de lookup AND for a de→ru lookup: the learner studies German either way.
  E.g. explanation_language=ru, word="aufstehen", source=de, target=ru → synonyms must be German
  ("sich erheben", "hochkommen") — NOT Russian ("подниматься").
  E.g. explanation_language=ru, source=ru "Противный человек" → target=de "unangenehmer Mensch":
...
""",
"check_error": """
You grade a German "Finde den Fehler" exercise at B2+. The learner is shown a faulty sentence as a
list of word tokens and must tap the ONE wrong word and retype it corrected. Input JSON:
{"woerter": ["...","..."], "index": <int>, "user": "<the learner's replacement word>"}.
The learner claims the token at position `index` (0-based) is wrong and should become `user`.
Decide whether that is a LEGITIMATE correction. It is correct (match=true) ONLY if BOTH hold:
(a) the ORIGINAL token at `index` is genuinely grammatically wrong in this sentence (wrong case,
    article, adjective ending, verb form, preposition, gender, auxiliary, or spelling);
(b) replacing ONLY that token with `user` yields the correct German form at that position — `user`
    is the right form there (ignore capitalization and ß↔ss differences).
The sentence MAY contain OTHER errors elsewhere; judge ONLY the token at `index`. Do NOT require the
whole sentence to become perfect — only that THIS word was wrong and `user` fixes THIS word. So a
learner who spots a REAL error other than the intended one must still be credited.
REJECT (match=false) if the original token at `index` was already correct, or if `user` is not the
right form for that slot.
Return STRICT JSON ONLY:
...
async def run_example_translation_repair(sentence: str, current: str, *,
                                         studied_language: str = "de",
                                         explain_language: str = "ru") -> str:
    """Естественный перевод фразы-примера вместо буквального. Пустая строка — не вышло.

    Зовётся УБОРКОЙ накопленного (scripts/word_diff_fix_tautologies.py), а не живой
    выдачей: на выдаче тавтология только считается (reason «tautology»), потому что
    правило-сторож узкое, но не безошибочное, и платить за перезапрос на каждом показе
    мы не будем, не увидев числа.
    """
    task_name = "example_translation_repair"
    content = await llm_execute(
        task_name=task_name,
        system_instruction_key=task_name,
        user_message=json.dumps(
            {"sentence": str(sentence or "").strip(),
             ...
             "studied_language": (studied_language or "de").strip().lower(),
             "explain_language": (explain_language or "ru").strip().lower()},
            ensure_ascii=False,
        ),
        poll_interval_seconds=2.0,
    )
    answer = parse_llm_json_object(content, context=task_name)
    return " ".join(str((answer or {}).get("translation") or "").split())
...
async def run_word_usage_enrichment(
    ...
    *,
    ...
    """«Полнота слова»: возвратность, управление, предлоги, сочетания, родня по приставкам.

    Работает по ОДНОМУ слову и ничего не сравнивает. Результат ложится в базу на само
    слово и служит всем — сравнению, поиску, тренировкам. Номера значений здесь НЕ
    присваиваются: их ставит наш код при сохранении, иначе после переобогащения ссылки
    начнут указывать на другие значения.
    """
    task_name = "word_usage_enrichment"
    payload = {
...
async def run_check_error(*, woerter: list[str], index: int, user: str) -> dict:
    """Bounded yes/no LLM check for a 'Finde den Fehler' miss: did the learner tap a
    GENUINELY wrong word and retype it correctly? Judges ONLY the tapped token, so a
    learner who spots a real error OTHER than the intended one — or supplies a valid
    variant the pool's `aliases` misses — is still credited. Only called as a fallback
    when the deterministic index+correction match misses."""
    content = await llm_execute(
        task_name="check_error",
        system_instruction_key="check_error",
        user_message=json.dumps(
            {"woerter": [str(w) for w in (woerter or [])], "index": int(index), "user": str(user)},
            ensure_ascii=False,
        ),
        poll_interval_seconds=1.0,
        responses_timeout_seconds=6.0,
...
def run_phrase_grammar_verdict(*, text: str, kind: str = "sentence",
    ...
    system = (
        ...
        "entry, not running text. Never answer verdict=\"error\" for it, and never return "
        "a `corrected` that differs from the input only by end punctuation - that is an "
        "empty complaint and it wastes the reader's time.\n"
        "- Never claim an error you cannot fix: if your `corrected` would be identical to "
        "the input, the verdict is \"ok\". Never write a `why` that replaces a fragment "
        "with the very same fragment.\n"
...
def _coerce_story_explanation(raw: str) -> dict:
    """Parse the structured 7-sentence story review JSON into a validated dict.
    Tolerant of code fences / stray prose; always returns the full shape so the
    modal never breaks."""
    s = (raw or "").strip()
    if s.startswith("```"):
        s = re.sub(r"^```[a-zA-Z]*\n?", "", s)
    ...
    if a >= 0 and b > a:
        try:
            ...
            data = None
    empty = {
        "summary": (raw or "").strip() if not isinstance(data, dict) else "",
        "sentences": [],
        "pattern": {"detected": False, "title": "", "explanation": "", "examples": []},
        "grammar_focus": [],
        "practice": [],
    ...
    def _errors(raw_errors) -> list:
        out = []
        for e in (raw_errors or [])[:4]:
            if not isinstance(e, dict):
                continue
            etype = _str(e.get("type")).lower()
            if etype not in _ALLOWED_EXPLAIN_ERROR_TYPES:
    ...
    def _alts(raw_alts) -> list:
        out = []
        for a_ in (raw_alts or [])[:2]:
            if isinstance(a_, dict) and _str(a_.get("variant")):
                out.append({"variant": _str(a_.get("variant")), "note": _str(a_.get("note"))})
            elif isinstance(a_, str) and a_.strip():
                out.append({"variant": a_.strip(), "note": ""})
        return out

    def _syns(raw_syns) -> list:
        out = []
        for sy in (raw_syns or [])[:3]:
            if not isinstance(sy, dict):
                continue
            word = _str(sy.get("word"))
            options = [_str(o) for o in (sy.get("options") or []) if _str(o)][:3]
            if word and options:
                out.append({"word": word, "options": options})
        return out

    sentences = []
    for idx, sent in enumerate((data.get("sentences") or [])[:7], start=1):
        if not isinstance(sent, dict):
            continue
        verdict = _str(sent.get("verdict")).lower()
        if verdict not in ("correct", "minor", "major"):
            verdict = "minor"
        try:
            s_index = int(sent.get("index") or idx)
        except (TypeError, ValueError):
            s_index = idx
        sentences.append({
            "index": s_index,
    ...
    raw_pattern = data.get("pattern") if isinstance(data.get("pattern"), dict) else {}
    pattern_examples = []
    for ex in (raw_pattern.get("examples") or [])[:3]:
        if isinstance(ex, dict) and (_str(ex.get("wrong")) or _str(ex.get("right"))):
            pattern_examples.append({
                "wrong": _str(ex.get("wrong")),
                "right": _str(ex.get("right")),
                "note": _str(ex.get("note")),
            })
    pattern = {
        "detected": bool(raw_pattern.get("detected")) and bool(_str(raw_pattern.get("explanation"))),
        "title": _str(raw_pattern.get("title")),
        "explanation": _str(raw_pattern.get("explanation")),
        "examples": pattern_examples,
    }

    grammar_focus = []
    for gf in (data.get("grammar_focus") or [])[:4]:
        if not isinstance(gf, dict) or not _str(gf.get("title")):
            continue
        gf_examples = []
        for ex in (gf.get("examples") or [])[:2]:
            if isinstance(ex, dict) and _str(ex.get("de")):
                gf_examples.append({"de": _str(ex.get("de")), "ru": _str(ex.get("ru"))})
        grammar_focus.append({
            "title": _str(gf.get("title")),
            "theory": _str(gf.get("theory")),
            "examples": gf_examples,
        })

    practice = []
    for pr in (data.get("practice") or [])[:3]:
        if isinstance(pr, dict) and _str(pr.get("ru")):
            practice.append({"ru": _str(pr.get("ru")), "hint": _str(pr.get("hint"))})
