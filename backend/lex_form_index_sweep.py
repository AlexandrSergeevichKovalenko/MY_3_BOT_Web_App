...
def _строчка_про_судью() -> str:
    """Одна строка про вторую ночную работу — чистку чужих переводов.

    Владелец получает ОДНО письмо, а не два: он и так сказал, что всё, что нужно
    вызывать командой, он забудет. Два письма про соседние вещи — тот же забытый вызов.
    """
    from backend.database import get_db_connection_context
    ...
    try:
        with get_db_connection_context() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT count(*), coalesce(sum(demoted), 0),
                              count(*) FILTER (WHERE verdict = 'ответы разошлись')
                         FROM bt_3_translation_judgements;"""
                )
                слов, убрано, спорных = cur.fetchone()
    except Exception:
        logging.exception("отчёт: судья переводов не ответил")
        return "Про чужие переводы: посчитать не удалось, смотри лог."
    if not слов:
        return ("Ночная чистка чужих переводов только запускается — числа будут "
                "в следующем письме.")
    return (f"Ещё ночью убираем русские слова, которые немецкому слову не подходят "
            f"(«отдыхает» у «rasen»). Разобрано слов: <b>{слов}</b>, убрано лишних "
            f"значений: <b>{убрано}</b>. Спорных, где два спроса модели не сошлись и "
            f"мы ничего не трогали: {спорных}.")
