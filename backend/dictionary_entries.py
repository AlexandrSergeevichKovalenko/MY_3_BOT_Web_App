...
class DictionaryLayerUnavailable(RuntimeError):
    """Слой статей не смог спросить базу. Это НЕ «слова нет».

    ⛔ ЗАЧЕМ ОТДЕЛЬНОЕ ИСКЛЮЧЕНИЕ, А НЕ ПУСТОЙ СПИСОК.

    Здесь стояло `except Exception: return []`. Пустой список означает «мы честно не
    знаем этого слова» — и по нему вызывающие идут к машинному переводчику, к модели,
    к платному разбору. При сбое связи с базой наружу уходил ТОТ ЖЕ пустой список, то
    есть обрыв сети превращался в «слова нет» и в тихую подмену словарного ответа
    машинным. Это ровно запрет из правила ноль: «Пустой результат от ошибки НЕ ОТЛИЧИМ
    от пустого результата от „данных правда нет“ — а это два разных мира».

    Поймано 02.09.2026 на живом замере: слой вернул пустой список для слова «Kugel»,
    которое в базе есть, и я чуть не объявил регресс продукта. Владелец: «чинить».

    Пустой список остаётся честным ответом «не знаем». Сбой теперь называется вслух.
    """
