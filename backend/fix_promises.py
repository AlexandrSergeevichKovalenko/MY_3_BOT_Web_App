...
# Приметы ВИДИМОГО перевода на карточке слова. Обе обязаны стоять в живом CSS.
# Жалоба владельца 15.09.2026 («Стендап дня», слово auf Kip stehen): перевод «невидим и
# сливается с текстом». Причина была одна — перевод набирался теми же чернилами
# --wn-text-2, что и пояснение «Как употреблять» под ним, и отличался от него только
# кеглем 18 против 15. Глаз читал его как пояснение, а не как ОТВЕТ карточки.
_WN_RU_VISIBLE = (
    ("color:var(--wn-text)", "чернила заголовка, а не серые пояснения"),
    ("font-weight:700", "жирность ответа, а не текста"),
)


def _worldnews_card_ru_faint() -> int:
    """Сколько примет ВИДИМОГО перевода пропало из живого CSS карточки слова. Обещано: 0.

    Карточка одна на обе рубрики — «Новость дня» и «Стендап дня»: рубрика меняет только
    заголовок экрана и пометы, само правило .worldnews-card-ru общее. Поэтому одна
    проверка закрывает оба экрана. Нет самого селектора — это не собранный фронт, и
    исход «не измерено», а не «0»."""
    import re
    css = _served_webapp_css()
    m = re.search(r"\.worldnews-card-ru\s*\{([^}]*)\}", css)
    if not m:
        raise LookupError("в CSS нет .worldnews-card-ru — это не собранный фронт")
    тело = re.sub(r"\s+", "", m.group(1))
    return sum(1 for примета, _ in _WN_RU_VISIBLE if примета not in тело)


def _worldnews_card_ru_screen() -> str:
    """Экран «после»: каким перевод приходит на телефон прямо сейчас."""
    import re
    css = _served_webapp_css()
    m = re.search(r"\.worldnews-card-ru\s*\{([^}]*)\}", css)
    if not m:
        raise LookupError("в CSS нет .worldnews-card-ru — это не собранный фронт")
    тело = re.sub(r"\s+", "", m.group(1))
    нет = [подпись for примета, подпись in _WN_RU_VISIBLE if примета not in тело]
    кегль = re.search(r"font-size:(\d+)px", тело)
    if not нет:
        return ("📖 Карточка слова («Новость дня» и «Стендап дня»): перевод набран чернилами "
                "заголовка, жирный, кегль " + (кегль.group(1) if кегль else "?") + " — он "
                "виден раньше, чем человек начнёт читать пояснения, и не сливается с "
                "«Как употреблять».")
    return ("📖 Карточка слова: перевод снова блёклый — потеряно " + str(len(нет))
            + " из 2 примет (" + ", ".join(нет) + "). Он сливается с пояснением под ним.")
...
# ── Клавиатура не таскает быстрый словарь (16.09.2026) ───────────────────────────────
# Повод (владелец, 16.09.2026, видео 10-54-35): встаёшь в поле ввода — шапка «Быстрый
# словарь» и кнопки вкладок уезжают за верх экрана, строка языков налезает на часы,
# «Вставить» и «Перевести» прячутся под клавиатуру. «Расхлябанный вид, нестабильный».
# Причина не в вёрстке, а в устройстве айфона: при выезде клавиатуры окно СТРАНИЦЫ не
# уменьшается, поэтому WebKit прокручивает всё вверх, лишь бы поле оказалось над
# клавиатурой. Починка — как у Яндекс.Переводчика: панель живёт по ВИДИМОЙ высоте окна
# (visualViewport), а не по высоте страницы, и стартовому экрану запрещено сжимать
# что-либо, кроме самого поля ввода.
#
# Меряем не исходник, а то, что вправду отдано телефону: кусок словаря и стили сайта.
_DICT_VIEWPORT_GUARDS = (
    ("--dq-vh", "замер видимой высоты окна (--dq-vh) в куске словаря"),
    ("data-dq-panel", "отметка панели (data-dq-panel) в куске словаря"),
)
_DICT_LAYOUT_GUARDS = (
    ("[data-dq-panel]", "правило «панель прибита к видимой полосе» в стилях"),
    ("dq-card--compose", "правило «стартовый экран помещается целиком» в стилях"),
)


def _dict_keyboard_guards_missing() -> list[str]:
    """Каких сторожей НЕТ в живой сборке быстрого словаря."""
    кусок = _served_webapp_chunk("DictionaryOverlay")
    стили = _served_webapp_css()
    нет = [подпись for метка, подпись in _DICT_VIEWPORT_GUARDS if метка not in кусок]
    нет += [подпись for метка, подпись in _DICT_LAYOUT_GUARDS if метка not in стили]
    return нет


def _dict_keyboard_drags_the_screen() -> int:
    """Сколько сторожей «клавиатура не таскает экран» потеряно. Обещано: 0."""
    return len(_dict_keyboard_guards_missing())


def _dict_keyboard_screen() -> str:
    """Экран «после»: что про поле ввода словаря отдаёт сайт прямо сейчас."""
    нет = _dict_keyboard_guards_missing()
    if нет:
        return ("⚠️ Быстрый словарь снова может уехать под клавиатуру: "
                + "; ".join(нет))
    return ("⌨️ Встаёшь в поле — экран словаря стоит на месте: шапка, вкладки и строка "
            "языков видны, «Вставить» и «Перевести» остаются прямо над клавиатурой, "
            "поле ввода само становится ниже. Ничего не прокручивается и не налезает "
            "на часы — как в Яндекс.Переводчике.")
...
# ── Карточка слова: перевод не раздавить, высота колоды не угадана (16.09.2026) ───────
# Повод (владелец, 16.09.2026, снимок «Слово 27 из 27», «aufs Auto angewiesen sein»): на
# карточке есть немецкое слово и нет русского перевода. Перевод не пропадал — от строки в
# 26 точек осталась полоска в 3: её СЖАЛО. Два слоя причины, оба записаны сюда:
#   1) высота колоды бралась формулой «экран минус 166 точек». 166 подобрали 04.09.2026
#      под «Новость дня» с заголовком рубрики; на экране видеоурока заголовка нет, сверху
#      занято ~75 точек — коробка выходила на короче экрана на 78 точек (под кнопкой
#      «Смотреть видео» на снимке пустует 78 точек), и середине карточки не хватило ~23;
#   2) середину было РАЗРЕШЕНО сжимать. Chrome такую нехватку прокручивает, WebKit на
#      айфоне давит первый элемент колонки — им оказался перевод (цитата и «как
#      употреблять» свёрстаны как grid и не поддались, весь дефицит лёг на одну строку).
# Меряем не исходник, а то, что вправду отдано телефону: стили живого сайта и кусок App,
# в котором едет сам замер. Нет сети или куска — «не измерено», а не «0».
_WN_DECK_MEASURED = "var(--wn-deck-max"          # потолок колоды берётся из замера, а не только из константы
_WN_BODY_NO_SHRINK = r"\.worldnews-card-body>\*\{[^}]*flex:0 0 auto"
_WN_CARD_NO_SHRINK = r"\.worldnews-card>\*:not\(\.worldnews-card-body\)\{[^}]*flex-shrink:0"


def _worldnews_card_guards_missing() -> list[str]:
    """Каких сторожей карточки слова НЕТ в том, что живой сайт отдаёт телефону."""
    import re
    css = _served_webapp_css()
    нет = []
    if _WN_DECK_MEASURED not in css:
        нет.append("высота колоды снова только из константы (нет var(--wn-deck-max))")
    if not re.search(_WN_BODY_NO_SHRINK, css):
        нет.append("середину карточки снова можно сжать (нет flex:0 0 auto у её строк)")
    if not re.search(_WN_CARD_NO_SHRINK, css):
        нет.append("заголовок и кнопки карточки снова можно сжать (нет flex-shrink:0)")
    if "--wn-deck-max" not in _served_webapp_chunk("App"):
        нет.append("замер высоты не доехал до телефона (в куске App нет --wn-deck-max)")
    return нет


def _worldnews_card_can_crush_translation() -> int:
    """Сколько сторожей карточки слова потеряно в живой сборке. Обещано: 0."""
    return len(_worldnews_card_guards_missing())


def _worldnews_card_screen() -> str:
    """Экран «после»: что про карточку слова отдаёт сайт прямо сейчас."""
    нет = _worldnews_card_guards_missing()
    if нет:
        return ("⚠️ Карточка слова снова может раздавить перевод: " + "; ".join(нет)
                + ".\nОткрой «Слова» в видеоуроке: под немецким словом должен стоять "
                  "русский перевод целиком, а не полоска.")
    return ("📐 Высоту карточки считает сама страница: сколько занято сверху и снизу — "
            "столько и вычитается, константы «минус 166 точек» больше нет.\n"
            "🚫 Ни одна строка карточки не сжимается: не влезло — середина прокручивается, "
            "а перевод остаётся строкой, а не полоской.")
...
# ── «Слова со вчерашних тренировок»: панель занимает весь экран (15.09.2026) ─────────
# Жалоба владельца (скриншот интерактива): «а почему мы не используем всё пространство
# экрана? 20% экрана гуляет». Замер на стенде с настоящим wordPick.css: низ панели стоял
# на 120 px выше низа экрана на ЛЮБОМ телефоне — 375×629: 79,0% высоты, 393×720: 81,7%,
# 430×800: 83,5%, 360×560: 76,4%. После правки — 97,1–98,0% на тех же четырёх.
#
# Корень: 05.09 правила экрана Space Rep перенесли из App.css в wordPick.css СЛОВО В
# СЛОВО, включая `height: calc(100dvh - 132px)`. В приложении это вычитание тут же
# перебивается `.webapp-page.is-fsrs-study .fsrs-study-screen` (height:auto + flex:1), а
# перебивку скрипт не взял: её селектор про страницу приложения, которой в оверлее
# интерактивов нет. Вычитание осталось голым — и съело 132 px ни на что.
#
# Обещание меряет не исходник, а CSS, который получает телефон: в нём не должно остаться
# ни одного правила, прибивающего высоту этой панели к числу. Вернётся — придёт письмом.
_WP_SCREEN_SELECTOR = ".wp-root .fsrs-study-screen"


def _count_wordpick_screen_pinned_height(css: str) -> int:
    """Сколько правил в живом CSS прибивают высоту панели интерактива к числу. Обещано: 0.

    Считается любое `height`/`max-height` с вычитанием из высоты экрана (calc(100dvh - …))
    у селектора .wp-root .fsrs-study-screen: именно оно оставляло полосу пустоты внизу.
    Заодно требуется сама растяжка (flex с ростом): без неё панель снова не дотянется до
    низа, даже если вычитание убрали. Нет самого селектора .wp-root — это не собранный
    фронт, считать нечего: исход «не измерено», а не «0»."""
    import re
    сжатый = re.sub(r"\s+", "", css)
    if ".wp-root" not in сжатый:
        raise LookupError("в CSS нет .wp-root — это не собранный фронт")
    прибито = 0
    растяжка = False
    for m in re.finditer(r"([^{}]+)\{([^}]*)\}", css):
        селекторы = [re.sub(r"\s+", " ", x).strip() for x in m.group(1).split(",")]
        if _WP_SCREEN_SELECTOR not in селекторы:
            continue
        тело = re.sub(r"\s+", "", m.group(2))
        if re.search(r"(^|;)(max-)?height:calc\(100dvh-", тело):
            прибито += 1
        if re.search(r"(^|;)flex:1[ ;]?", тело) or re.search(r"(^|;)flex:11auto", тело):
            растяжка = True
    if not растяжка:
        # Растяжки нет — панель не дотянется до низа, и это ровно тот же дефект.
        прибито += 1
    return прибито


def _wordpick_screen_pinned_height() -> int:
    """Правил, прибивающих высоту панели «Слова со вчерашних тренировок». Обещано: 0."""
    return _count_wordpick_screen_pinned_height(_served_webapp_css())


def _wordpick_screen_height_screen() -> str:
    """Экран «после»: что по этому поводу отдаёт сайт прямо сейчас."""
    n = _count_wordpick_screen_pinned_height(_served_webapp_css())
    if n == 0:
        return ("📐 «Слова со вчерашних тренировок»: панель тянется на всю высоту экрана "
                "(97–98% вместо 76–83%). Полосы пустоты под кнопками оценки больше нет "
                "ни на большом телефоне, ни на маленьком.")
    return ("📐 «Слова со вчерашних тренировок»: в живом CSS снова " + str(n) + " правил(о), "
            "прибивающих высоту панели к числу. Под кнопками оценки вернулась пустая полоса.")
...
def _relation_gap_silent_slots() -> int:
    """Слотов, в которые задание МОГЛО уйти людям, но не ушло ни одному. Обещано: 0.

    Слот — это (день, вид): синонимы в 14:45, антонимы в 17:00. «Могло» — значит
    выполнены оба условия разом: позавчера тренировка отправила слово этого вида И его
    получил хоть один человек. Если при этом за день нет ни одной строки в
    bt_3_relation_gap_dispatches для не-админа, слот промолчал.
    ...
    ┌─ ИСПРАВЛЕНО 15.09.2026. СЧИТАЛИ СЛОВА, А НАЗЫВАЛИ ДНЯМИ. ────────────────────┐
    │ Здесь стояло COUNT(*) по соединению дней с банком — то есть считалась КАЖДАЯ  │
    │ строка банка, у которой trainer_sent_date = день минус два. Утром 15.09       │
    │ владельцу ушло «сейчас 4», хотя считался РОВНО ОДИН день (окно = с 14.09 по   │
    │ вчера), и больше единицы там быть не могло физически.                        │
    │ Разложение сырого числа 4 на живой базе: все четыре — это 14.09, вид          │
    │ «антонимы», четыре РАЗНЫХ слова с отметкой тренировки за 12.09 (stabil,       │
    │ erweitern, bedeutsam, ehrlich). Слот один, слово уходит одно, молчание одно.  │
    │ Синонимы 14.09 не попали в счёт законно: рассылка была (1 человек).           │
    │ Причина, почему слов на одну дату много: кроме общей рассылки в 11:00/16:00    │
    │ тренировку раздаёт ЛИЧНАЯ ротация (slot_hour 3 и 4), и каждая её выдача тоже  │
    │ ставит trainer_sent_date. За 12.09 таких антонимов вышло четыре.              │
    │ Теперь считаются РАЗНЫЕ слоты (DISTINCT день+вид), поэтому число не может     │
    │ превысить длину окна.                                                        │
    │ Перемерить: SELECT DISTINCT trainer_sent_date, relation, count(*) FROM        │
    │ bt_3_sprint_bank WHERE trainer_sent_date >= CURRENT_DATE - 4 GROUP BY 1,2.    │
    └──────────────────────────────────────────────────────────────────────────────┘
    """
    from backend.database import get_db_connection_context
    with get_db_connection_context() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                moglo AS (
                    -- DISTINCT: слот один на (день, вид). Без него считалась КАЖДАЯ
                    -- строка банка с этой отметкой, и измеритель выдавал число больше
                    -- длины окна: 15.09.2026 — «4» при одном возможном дне, а молчавший
                    -- слот на живой базе был РОВНО ОДИН (14.09, антонимы). См. рамку выше.
...
def _trainer_bank_thin() -> int:
    ...
    ┌─ ПРОВЕРЕНО 15.09.2026 (второе утро). НЕ ПОДНИМАТЬ КАК НОВУЮ НАХОДКУ. ─────────┐
    │ Замер на живой базе: синонимы — банк 40, свободно спринт 22 / тренировка 6    │
    │ при пороге 4 → ХВАТАЕТ (сутки назад было 1 при пороге 4, то есть добор        │
    │ сработал). Антонимы — банк 52, свободно спринт 36 / тренировка 5 при пороге 6 │
    │ → не хватает РОВНО ОДНОГО слова, и это те самые «1» в отчёте. Ночь 15.09      │
    │ добрала 11 синонимов и 8 антонимов, сборка тренажёра прошла в 03:41.          │
    │ Вывод: механизм работает, красное второе утро — доборка запаса, а не поломка. │
    │ Красное ТРЕТЬЕ утро (16.09) означало бы, что добор не поднимает именно        │
    │ trainer-свободные слова, и вот тогда это дефект.                              │
    │ Перемерить: count_available_trainer_items(relation=..., cooldown_days=21)     │
    │ против decide_topup(...).floor_free по обоим видам.                           │
    └──────────────────────────────────────────────────────────────────────────────┘
    """
...
def _allowed_rows_not_real_people() -> int:
    ...
    а владелец видел это как «🚫 Не дошло». 16.09.2026 правило перенесено В САМУ ЗАПИСЬ:
    страж refuse_if_not_a_person на всех дверях плюс ограничение в таблице.

    Сначала проверяется ЗАМОК, потом число. Ноль при снятом замке — правда ровно до первой
    записи, и выдавать его за «держится» нельзя: это тот самый случай, когда пустой
    результат от поломки неотличим от пустого результата от «данных правда нет»."""
    from backend.database import (count_allowed_rows_not_real_people,
                                  allowed_users_person_check_is_in_place,
                                  allowed_users_person_check_name)
    if not allowed_users_person_check_is_in_place():
        raise RuntimeError(
            f"запрет {allowed_users_person_check_name()} снят с таблицы bt_3_allowed_users: "
            f"писать строки без человека снова может кто угодно. Мерить число бессмысленно, "
            f"пока замка нет")
    return count_allowed_rows_not_real_people()


def _allowed_rows_screen() -> str:
    """Экран владельца «после»: КТО именно лежит в списке доступа без человека.

    Одного числа мало — «2» не говорит, откуда они. Пометка и дата называют дверь, а
    решение об удалении принимает владелец (scripts/allowed_users_drop_test_rows.py
    --apply). Деплой сам ничего не чистит: молчание согласием не считается."""
    from html import escape
    from backend.database import list_allowed_rows_not_real_people, count_allowed_users
    строки = list_allowed_rows_not_real_people()
    шапка = f"👥 Живых людей в списке доступа: <b>{count_allowed_users()}</b>"
    if not строки:
        return шапка + "\nСтрок без человека: <b>0</b> — чисто."
    куски = [шапка, f"Строк, за которыми НЕ может быть человека: <b>{len(строки)}</b>", ""]
    for r in строки:
        куски.append(f"• <code>{r['user_id']}</code> — {escape(str(r['note'] or '—'))}"
                     f" (создана {r['created_at']:%d.%m.%Y %H:%M})")
    куски.append("")
    куски.append("Новых таких строк появиться больше не может: правило стоит в самой записи. "
                 "Эти — накопленные; убрать их: "
                 "<code>python3 scripts/allowed_users_drop_test_rows.py --apply</code>")
    return "\n".join(куски)
...
PROMISES: tuple[Promise, ...] = (
    ...
    Promise(
        key="worldnews_card_never_crushes_translation",
        title="Потерянных сторожей: карточка слова снова может раздавить русский перевод",
        since="16.09.2026",
        expected=0,
        measure=_worldnews_card_can_crush_translation,
        screen=_worldnews_card_screen,
        how="/admin_promises — или backend.fix_promises._worldnews_card_guards_missing(). "
            "Читает стили и кусок App ЖИВОГО сайта. До 16.09.2026 высота колоды бралась "
            "формулой «экран минус 166 точек» (подобрана 04.09.2026 под «Новость дня» с "
            "заголовком рубрики). На экране видеоурока заголовка нет: сверху занято ~75 "
            "точек, коробка выходила на короче экрана на 78 точек, середине карточки не "
            "хватало ~23 — и WebKit на айфоне вместо прокрутки сжимал первую строку "
            "середины, перевод: 26 точек превращались в 3 (снимок владельца «aufs Auto "
            "angewiesen sein»). Проверено на стенде 393×735: замер отдаёт колоде ровно "
            "остаток экрана, страница не прокручивается, кнопка «Смотреть видео» на месте. "
            "Число ВЫРОСЛО = либо вернулась константа вместо замера, либо со строк карточки "
            "сняли запрет сжиматься",
    ),
    Promise(
        key="quick_dict_keyboard_never_drags_the_screen",
        title="Потерянных сторожей: клавиатура снова может утащить экран быстрого словаря",
        since="16.09.2026",
        expected=0,
        measure=_dict_keyboard_drags_the_screen,
        screen=_dict_keyboard_screen,
        how="/admin_promises — или backend.fix_promises._dict_keyboard_guards_missing(). "
            "Читает кусок DictionaryOverlay и стили ЖИВОГО сайта. До 16.09.2026 панель "
            "жила по высоте СТРАНИЦЫ (100dvh), а айфон при выезде клавиатуры страницу не "
            "уменьшает — он её прокручивает: шапка «Быстрый словарь» и вкладки уезжали за "
            "верх экрана, строка языков налезала на часы, «Перевести» пряталась под "
            "клавиатуру (видео владельца 10-54-35). Теперь высота берётся от видимой части "
            "окна (visualViewport), панель прибита к ней, а на стартовом экране сжиматься "
            "разрешено ТОЛЬКО полю ввода. Проверено на стенде 390×538 (столько видно на "
            "айфоне при клавиатуре): панель 538, содержимое 538, прокрутки ноль, шапка на "
            "месте, «Перевести» на 489-й точке. Число ВЫРОСЛО = либо вернулась высота по "
            "странице, либо со стартового экрана сняли запрет сжиматься",
    ),
    Promise(
    ...
    Promise(
        ...
        screen=_allowed_rows_screen,
        how="python3 scripts/allowed_users_drop_test_rows.py --dry-run — ждём 0, и он же "
            "называет строки поимённо. ИСТОРИЯ: 14.09.2026 их было три (77, 777, "
            "987654321), писал ПРОГОН ТЕСТОВ по боевой базе; 15.09 поставлены два замка "
            "(SKIP_ACCESS_GRANT_WRITES в conftest и проверка is_real_telegram_user_id) — "
            "и 16.09 число всё равно было 2. Разбор 16.09: замки на месте, но стояли на "
            "ОДНОЙ двери из пяти. Остальные четыре — /allow, кнопка «одобрить», посев "
            "админов при старте бота и ночной впуск из очереди — писали мимо правила, "
            "а шестым путём был нагрузочный прогон со своим подключением. По решению "
            "владельца правило перенесено В САМУ ЗАПИСЬ: refuse_if_not_a_person на всех "
            "дверях плюс ограничение в таблице bt_3_allowed_users_person_* (NOT VALID: "
            "накопленное не трогается, новое не проходит). Число ВЫРОСЛО = ограничение "
            "сняли с таблицы — проверить: SELECT conname FROM pg_constraint WHERE "
            "conrelid = 'bt_3_allowed_users'::regclass",
    ),
    Promise(
    ...
    Promise(
        ...
        since="15.09.2026",
        expected=0,
        measure=_relation_gap_silent_slots,
        screen=_relation_gap_screen,
        how="/admin_promises — считаются слоты (день + вид: синонимы 14:45, антонимы "
            "17:00) с 14.09.2026, и только те, где позавчера тренировка отправила слово "
            "и его получил хоть один человек. Ждём 0. Не ноль = сборка заготовок упала "
            "или рассылка не дошла. 15.09.2026 сырое число было 4 при окне в один день: "
            "считались слова банка, а не слоты (разбор в докстроке измерителя). Живая "
            "причина того единственного слота (14.09, антонимы): слово «ehrlich» "
            "тренировал ОДИН человек со «своими часами» (Pro, окно 06–09 и 18–22:30), а "
            "слот стоит в 17:00 — таких людей рассылка по слотам не берёт вовсе, их ведёт "
            "капля (_drip_bonus_pass), и она умела спринт и повтор, но НЕ «Подставь "
            "синоним». ИСПРАВЛЕНО 15.09.2026 по решению владельца: капля несёт все три "
            "сверхплановые ступеньки, у двух видов разные часы слота (6 и 7), стережёт "
            "backend/tests/test_drip_carries_the_whole_rail.py",
    ),
    Promise(
    ...
    Promise(
        key="worldnews_card_ru_visible",
        title="Перевод на карточке слова («Новость дня» и «Стендап дня») виден, а не сливается с текстом",
        since="15.09.2026",
        expected=0,
        measure=_worldnews_card_ru_faint,
        screen=_worldnews_card_ru_screen,
        how="открыть $WEB_APP_URL, скачать подключённые .css; в правиле .worldnews-card-ru "
            "должны стоять color:var(--wn-text) и font-weight:700 — ждём 0 пропавших. "
            "Глазами: стендап дня → «Слова» → перевод под немецким словом чёрный и жирный, "
            "заметно темнее и крупнее строки «Как употреблять»",
    ),
    Promise(
    ...
    Promise(
        key="wordpick_screen_uses_full_height",
        title="Правил, прибивающих высоту панели «Слова со вчерашних тренировок» к числу "
              "(полоса пустоты под кнопками оценки)",
        since="15.09.2026",
        expected=0,
        measure=_wordpick_screen_pinned_height,
        how="python3 -c \"from backend.fix_promises import _wordpick_screen_pinned_height as f; print(f())\" "
            "— скачивает живой CSS сайта и ищет у '.wp-root .fsrs-study-screen' height:calc(100dvh-…) "
            "и наличие flex-растяжки. Руками — открыть постер «Слова со вчерашних тренировок», "
            "нижний край панели должен совпадать с нижним краем экрана",
        screen=_wordpick_screen_height_screen,
    ),
    Promise(
