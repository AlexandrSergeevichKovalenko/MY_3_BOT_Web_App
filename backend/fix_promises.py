...
# Сторожа двух родов, и второй род важнее первого. «ДОЛЖНО БЫТЬ» — то, без чего правка
# не работает. «НЕ ДОЛЖНО БЫТЬ» — то, что уже пробовали 16.09.2026 и что владелец увидел
# на экране как дефект. Попыток было три, и ошибочны обе первые:
#   1) fixed + ручной сдвиг на visualViewport.offsetTop → карточка съезжала вниз и
#      возвращалась (видео 11-36-42: 28 движущихся кадров из 89);
#   2) убрали fixed совсем → страницу подкрутило вверх на ~280 точек, шапки и вкладок не
#      осталось, снизу серая пустота (видео 12-53-06, кадр 30);
#   3) верно: fixed + top: 0 + запертая страница, БЕЗ компенсации сдвига.
# Поэтому data-dq-panel обязан БЫТЬ (это и есть слой с запертой страницей), а --dq-vtop
# обязан ОТСУТСТВОВАТЬ (это и есть ручной сдвиг). У Яндекса на том же покадровом замере
# 0 движущихся кадров из 63 — это цель.
_DICT_VIEWPORT_GUARDS = (
    ...
    # На Android клавиатура сжимает ОКНО целиком, а базовый кегль считался формулой от
    # высоты окна — весь словарь ужимался на четверть (18,46 → 13,91; «Быстрый словарь»
    # 14,4 → 10,9, замер 16.09.2026). Теперь кегль ставит скрипт и только пока клавиатуры
    # нет. Пропала метка — вернулось ужимание, и видно это только на Android.
    ("--ans-base", "кегль, замороженный на время клавиатуры (--ans-base) в куске словаря"),
)
_DICT_VIEWPORT_FORBIDDEN = (
    ("--dq-vtop", "вернулся ручной сдвиг панели (--dq-vtop) — от него карточка съезжает"),
)
_DICT_LAYOUT_GUARDS = (
    ("[data-dq-panel]", "правило «панель — отдельный слой, страница заперта» в стилях"),
    ("[data-dq-kbd]", "правило «под клавиатурой нет пустой ленты снизу» в стилях"),
    ("dq-card--compose", "правило «стартовый экран помещается целиком» в стилях"),
    # ГЛАВНЫЙ сторож. Пока белое место под полем — это подложка, а не само поле, айфону
    # нечего подкручивать при появлении клавиатуры. Вернётся поле на пол-экрана — вернётся
    # прыжок, и никакими слоями его уже не убрать (проверено тремя способами 16.09.2026).
    ("dq-compose-fill", "подложка под полем: без неё поле снова на пол-экрана и экран прыгает"),
)


def _dict_keyboard_guards_missing() -> list[str]:
    """Что не так с живой сборкой быстрого словаря: чего нет и что вернулось."""
    ...
    нет += [подпись for метка, подпись in _DICT_VIEWPORT_FORBIDDEN if метка in кусок]
    return нет
...
def _dict_keyboard_screen() -> str:
    ...
    return ("⌨️ Встаёшь в поле — экран словаря стоит на месте: шапка, вкладки и строка "
            "языков не двигаются, «Вставить» и «Перевести» остаются внутри карточки "
            "прямо над клавиатурой. Меняется одно — высота поля ввода, и один раз. "
            "Ни съездов, ни дрожи — как в Яндекс.Переводчике.")
...
def _genitive_phrases_without_article() -> int:
    ...
    В тот же день добавлена ТРЕТЬЯ ступень — модель, на то, чего не знает ни один
    справочник (решение владельца 16.09.2026). Число тут снова подрастёт, и это опять
    не новый дефект, а новые ответы. ⚠ Меряется СУХИМ прогоном, а он к модели НЕ ХОДИТ:
    читает только вердикты, уже купленные и лежащие в bt_3_genitive_head_article.
    Поэтому утренняя проверка обещания не может начать тратить деньги."""
...
# ── Разговор с моделью не идёт в закрытый цикл событий (17.09.2026) ───────────────────
# Повод (владелец, 16.09.2026): «очень много перебоев… словарь часто не работает». В
# логах прода за 5 часов — 17 × RuntimeError('Event loop is closed'). Быстрая половина
# разбора (повтора у неё нет) умирала за 1–2 мс, и человек ждал медленную: 9 запросов
# из 13 дольше 10 секунд, один 35. Причина: клиент OpenAI один на процесс и держит
# открытые соединения, а каждый веб-запрос открывал СВОЙ цикл событий (asyncio.run) и
# закрывал его на выходе; следующий запрос брал соединение от покойника.
#
# Обе половины починки проверяются здесь, потому что каждая закрывает свою дыру:
# один живой цикл на процесс (llm_loop) и клиент, привязанный к своему циклу.

def _llm_talks_into_a_closed_loop() -> int:
    """Сколько дыр «разговор с моделью в чужом цикле» открыто прямо сейчас. Обещано: 0.

    Считается по ЗАДЕПЛОЕННОМУ коду — проверка живёт в том же процессе, что и сервер,
    поэтому читает ровно те файлы, которые сейчас работают, а не то, что лежит в репо
    у агента."""
    ...
    сервер = Path(__file__).resolve().parent / "backend_server.py"
    текст = сервер.read_text(encoding="utf-8")
    дыры += len([1 for строка in текст.split("\n")
                 if _re.search(r"(?<![\w.])asyncio\.run\(", строка)])

    from backend.openai_manager import client as _client, _LoopBoundOpenAI
    if not isinstance(_client, _LoopBoundOpenAI):
        дыры += 1

    return дыры


def _llm_loop_screen() -> str:
    """Экран «после»: чем сейчас отвечает словарь на нажатие человека."""
    дыры = _llm_talks_into_a_closed_loop()
    if дыры:
        return ("⚠️ Разговор с моделью снова идёт через закрывающиеся циклы "
                f"({дыры} мест). Жди «перебоев»: быстрая половина разбора будет "
                "умирать за 1–2 мс, а человек — ждать медленную по 10–35 секунд.")
    return ("⚡ Словарь отвечает без перебоев: разговор с моделью идёт по одному живому "
            "соединению, которое больше не обрывается между запросами. Быстрая часть "
            "карточки приходит сразу, а не после падения и долгого повтора.")


PROMISES: tuple[Promise, ...] = (
    Promise(
        key="llm_never_talks_into_a_closed_loop",
        title="Разговор с моделью в закрытом цикле событий (перебои словаря)",
        since="17.09.2026",
        expected=0,
        measure=_llm_talks_into_a_closed_loop,
        screen=_llm_loop_screen,
        how="/admin_promises — или backend.fix_promises._llm_talks_into_a_closed_loop(). "
            "Считает в ЖИВОМ коде две дыры: места в backend_server.py, открывающие "
            "собственный цикл событий (asyncio.run), и клиент OpenAI, не привязанный к "
            "своему циклу. Замер до починки: 57 таких мест и общий клиент на все циклы; "
            "в логах прода 16.09.2026 за 5 часов — 17 падений «Event loop is closed», "
            "9 запросов из 13 дольше 10 секунд. После починки обе дыры закрыты: 0. "
            "Число выросло = кто-то вернул asyncio.run в веб-сервис, и перебои вернутся.",
    ),
    ...
    Promise(
        ...
        how="/admin_promises — или backend.fix_promises._dict_keyboard_guards_missing(). "
            ...
            "окна (visualViewport), панель — отдельный слой, страница заперта, а на "
            "стартовом экране сжиматься разрешено ТОЛЬКО полю ввода. Попыток было три: "
            "с ручным сдвигом на offsetTop карточка съезжала (28 движущихся кадров из 89), "
            "без слоя вовсе страницу подкрутило на ~280 точек и шапка исчезла. Поэтому "
            "сторожа двух родов: --dq-vh, data-dq-panel, [data-dq-kbd] и dq-card--compose "
            "должны БЫТЬ, а --dq-vtop НЕ должен. Проверено на стенде 390×538 (столько видно "
            "на айфоне при клавиатуре): панель 538, содержимое 538, прокрутки ноль, шапка "
            "на 86-й точке, кнопка внутри карточки. КОРЕНЬ дефекта нашёлся только с "
            "четвёртого захода: айфон обязан показать ПОЛЕ целиком, а поле было ростом в "
            "пол-экрана и нижним краем уходило под клавиатуру — подкрутка была неизбежна, "
            "и слои её не лечили. Теперь поле ростом в свой текст (44 точки вместо 393), а "
            "белое место даёт подложка dq-compose-fill; поле стоит на 271..315, клавиатура "
            "начинается на 494 — подкручивать нечего. Цель на видео — 0 движущихся кадров, "
            "как у Яндекса (0 из 63)",
    ),
    Promise(
