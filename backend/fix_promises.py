...
# на экране как дефект: панель фиксированным слоем и ручной сдвиг на visualViewport
# .offsetTop. От такой пары карточка съезжала вниз и возвращалась (28 кадров из 89
# против 0 из 63 у Яндекса). Если это вернётся в сборку — обещание нарушено.
...
_DICT_LAYOUT_FORBIDDEN = (
    ("data-dq-panel", "панель снова прибита фиксированным слоем (data-dq-panel) в стилях"),
)
...
def _dict_keyboard_guards_missing() -> list[str]:
    ...
    нет += [подпись for метка, подпись in _DICT_LAYOUT_FORBIDDEN if метка in стили]
    return нет
...
PROMISES: tuple[Promise, ...] = (
    ...
    Promise(
        ...
        how="/admin_promises — или backend.fix_promises._dict_keyboard_guards_missing(). "
            ...
            "окна (visualViewport), а на стартовом экране сжиматься разрешено ТОЛЬКО полю "
            "ввода. Первая попытка 16.09.2026 прибивала панель фиксированным слоем и "
            "двигала её на visualViewport.offsetTop — айфон во время анимации делает это "
            "сам, сдвиги складывались, и карточка съезжала вниз и возвращалась: 28 кадров "
            "движения из 89 против 0 из 63 у Яндекса, а «Перевести» вылезала за белый край. "
            "Поэтому сторожа два рода: --dq-vh и dq-card--compose должны БЫТЬ, а --dq-vtop "
            "и data-dq-panel НЕ должны — их возврат и есть нарушение. Проверено на стенде "
            "390×538 (столько видно на айфоне при клавиатуре): панель 538, содержимое 538, "
            "прокрутки ноль, шапка на 86-й точке, кнопка внутри карточки с запасом 29",
    ),
    Promise(
