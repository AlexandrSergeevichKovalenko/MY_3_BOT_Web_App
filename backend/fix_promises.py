...
PROMISES: tuple[Promise, ...] = (
    ...
    Promise(
        ...
        how="python3 scripts/allowed_users_drop_test_rows.py --dry-run — ждём 0. Кто их "
            "писал, выяснено 15.09.2026: ПРОГОН ТЕСТОВ по боевой базе. Тест патчит "
            "проверку подписи initData, идёт тестовым клиентом Flask в /api/webapp/* с "
            "выдуманным id, и сторож зовёт self-serve дверь; 14.09 в 19:02/19:05/19:07 и "
            "15.09 в 06:54/06:55/06:56 строки возвращались именно так, а владельцу шло "
            "«🆕 Новый пользователь». Поставлены два замка: прогон тестов не пишет ничего "
            "(SKIP_ACCESS_GRANT_WRITES, conftest) и дверь не впускает id, который по "
            "is_real_telegram_user_id не может быть человеком. Число ВЫРОСЛО = замок "
            "сняли, или пишет ещё какой-то путь — искать по note и created_at строки",
    ),
    Promise(
