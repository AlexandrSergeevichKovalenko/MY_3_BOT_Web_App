...
def подчистить_после_переименования(cur, *, unit_id: int, old_text: str,
    ...
    итог.update(_вернуть_в_круг_после_переименования(cur, unit_id=unit_id,
                                                     old_text=old_text, new_text=new_text))
    return итог


def _вернуть_в_круг_после_переименования(cur, *, unit_id: int, old_text: str,
                                         new_text: str) -> dict[str, int]:
    """Переименованное слово обязано пройти НАШУ процедуру заново, а не остаться на
    полпути.

    ┌─ НАЙДЕНО 27.08.2026 ПРИ РАЗБОРЕ ВОПРОСА ВЛАДЕЛЬЦА. ───────────────────────────┐
    │ «Если я исправлю заголовок, он ведь должен вернуться назад и пройти всю нашу   │
    │ процедуру от А до Я?» Не проходил. Переименование меняло написание, лемму,     │
    │ ключ, род, пул и кеш — но НЕ ЧАСТЬ РЕЧИ и не наши таблицы форм.                │
    │ Живой случай: «Finster» лежит у нас существительным со значением «темнота»,    │
    │ хотя это прилагательное «finster» (тёмный). Исправь заголовок — запись всё     │
    │ равно осталась бы существительным, программа снова пошла бы искать склонение   │
    │ существительного и слово вернулось бы в ту же очередь ПО КРУГУ.                 │
    │                                                                                │
    │ Часть речи берётся у ДВЕРИ СЛОВА (`german_word_gate.check_word`) и только      │
    │ когда та отвечает по справочнику. Молчит справочник — часть речи не трогаем:   │
    │ угадывать её нельзя, и слово честно вернётся вопросом.                          │
    └────────────────────────────────────────────────────────────────────────────────┘
    """
    итог = {"часть речи": 0, "отказ форм снят": 0, "снято с очереди форм": 0}
    голое = str(new_text or "").strip()
    части = голое.split(" ", 1)
    if len(части) == 2 and части[0].lower() in {"der", "die", "das"}:
        голое = части[1].strip()
    if not голое:
        return итог

    # 1. Часть речи — по справочнику, через дверь слова.
    #
    # ⚠ СЮДА ПРИХОДЯТ И ФРАЗЫ. Эту дочистку зовёт не только жалоба на слово, но и
    # решение по спорной ФРАЗЕ (`database.apply_phrase_review_decision`), а там
    # `new_text` — целое предложение. Дверь слова про одно слово, и своим сторожем она
    # такой вопрос теперь отклоняет сама (`german_word_gate.check_word`, 01.09.2026):
    # ответ приходит со статусом «не одно слово», в кеш не ложится и владельцу не идёт.
    # Копию проверки здесь заводить НЕЛЬЗЯ — ровно из-за копий у восьми вызывающих
    # девятый и остался без неё.
    try:
        from backend.german_word_gate import CONFIRMED, REPAIRED, check_word
        вердикт = check_word(голое, allow_network=True)
        новая = str((вердикт or {}).get("pos") or "").strip()
        источник = str((вердикт or {}).get("source") or "")
        if (новая and источник == "справочник"
                and str((вердикт or {}).get("status") or "") in (CONFIRMED, REPAIRED)):
            cur.execute("UPDATE bt_3_lex_units SET pos = %s, pos_source = %s "
                        " WHERE id = %s AND pos IS DISTINCT FROM %s;",
                        (новая, "справочник после переименования", int(unit_id), новая))
            итог["часть речи"] = cur.rowcount or 0
    except Exception:
        logging.warning("переименование: часть речи не пересчитана для %r", голое,
                        exc_info=True)

    # 2. Отрицательный вердикт форм по НОВОМУ написанию снимаем: он мог достаться от
    #    другого слова. Подтверждённые таблицы не трогаем — это данные справочника.
    for таблица, колонка in (("bt_3_german_noun_declensions", "noun"),
                             ("bt_3_german_adjective_degrees", "adjective")):
        cur.execute(f"DELETE FROM {таблица} WHERE {колонка} = lower(%s) "
                    f"  AND NOT documented;", (голое,))
        итог["отказ форм снят"] += cur.rowcount or 0

    # 3. Оба написания снимаем с очереди разбора форм: старого слова у нас больше нет,
    #    а новое пойдёт ночным путём с чистого листа.
    cur.execute("DELETE FROM bt_3_reference_forms_unresolved "
                " WHERE lower(word) IN (lower(%s), lower(%s));",
                (str(old_text or "").strip(), голое))
    итог["снято с очереди форм"] = cur.rowcount or 0
    return итог


def _пусто(значение: Any) -> bool:
    """Пустое значение в предложении модели — это «поле убрать», а не «нет ответа»."""
...
def _применить_правку(текущий: dict, правка: dict) -> dict[str, Any]:
    """Правка поверх разбора. Пустое значение ОЧИЩАЕТ поле, сохраняя его ТИП.

    ┌─ НАЙДЕНО 27.08.2026 НА ЖИВОМ РЕШЕНИИ. ────────────────────────────────────────┐
    │ Модель по «Finster» предложила {"forms": {}} — убрать таблицу форм, потому что │
    │ в карточке лежало склонение ФАМИЛИИ. Предложение законное, но по дороге его    │
    │ никто не понимал как «убрать»: экран печатал слово «станет» и пустоту после    │
    │ него, а сюда пустое значение уходило как есть — и в поле, где лежал СЛОВАРЬ    │
    │ форм, могла лечь пустая СТРОКА, если модель ответит "" вместо {}.              │
    │                                                                               │
    │ Правило одно на все поля: пустое значение очищает поле пустым значением ТОГО   │
    │ ЖЕ ТИПА, что лежит сейчас (словарь → {}, список → [], строка → ""). Тип поля   │
    │ карточки читают экраны, и менять его правкой нельзя.                          │
    └───────────────────────────────────────────────────────────────────────────────┘
    """
    итог: dict[str, Any] = {}
    for ключ, значение in (правка or {}).items():
        if not _пусто(значение):
            итог[ключ] = значение
            continue
        было = (текущий or {}).get(ключ)
        if isinstance(было, dict):
            итог[ключ] = {}
        elif isinstance(было, (list, tuple)):
            итог[ключ] = []
        elif isinstance(было, str):
            итог[ключ] = ""
        else:
            итог[ключ] = значение
    return итог
...
def apply_owner_decision(complaint_id: int, decision: str, *,
    ...
    if решение == "переименовать":
        ...
        новый.update(_применить_правку(новый, правка))
