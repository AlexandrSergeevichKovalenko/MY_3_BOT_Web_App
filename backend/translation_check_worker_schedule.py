...
def _railway_graphql(query: str, variables: dict) -> dict:
    graphql_url = os.getenv("TRANSLATION_CHECK_WORKER_RAILWAY_GRAPHQL_URL") or RAILWAY_GRAPHQL_URL
        ...
        try:
            ...
            if payload.get("errors"):
                if _railway_payload_is_not_authorized(payload) and not is_last:
                    logging.warning(
                        "translation_check_worker_schedule: Railway GraphQL auth rejected auth_mode=%s, retrying alternate auth mode",
                        auth_mode,
                    )
                    continue
            ...
            if status_code in {401, 403} and not is_last:
                logging.warning(
                    "translation_check_worker_schedule: Railway GraphQL HTTP auth rejected auth_mode=%s status=%s, retrying alternate auth mode",
                    auth_mode,
                    status_code,
                )
                continue
            ...
            if not is_last and "Not Authorized" in str(exc):
                logging.warning(
                    "translation_check_worker_schedule: Railway GraphQL payload auth rejected auth_mode=%s, retrying alternate auth mode",
                    auth_mode,
                )
                continue
            raise
