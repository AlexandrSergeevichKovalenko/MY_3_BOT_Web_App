...
def unavailable_reason() -> str:
    ...
    (сеть, квота, 500) ловится по-прежнему в review_new_card — здесь его не поймать
    иначе как лишним платным запросом.
    """
    ...
    if not str(os.getenv("GEMINI_API_KEY") or "").strip():
        return ("нет ключа второго голоса (GEMINI_API_KEY) в этом сервисе — "
