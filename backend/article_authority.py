    ...
    try:
        ...
        with get_db_connection_context() as conn:
            with conn.cursor() as cur:
                ...
                # ┌─ ПРОВЕРЕНО 25.08.2026. НЕ ПОДНИМАТЬ ЭТО КАК НОВУЮ НАХОДКУ. ──────┐
                # │ Запрос НАМЕРЕННО не смотрит ни на verified, ни на retired.        │
                # │                                                                   │
                # │ Что мерили: сколько написаний получают род от неподтверждённой     │
                # │ догадки модели, лежащей на строке банка.                          │
                # │ Сырое число 573 разложилось так:                                   │
                # │   224 — Wiktionary их всё равно знает, догадка не используется;    │
                # │   349 — Wiktionary молчит, в ответ уходит именно догадка:          │
                # │           268 справочник склонений знает и СОГЛАСЕН с ней;         │
                # │            72 не знает и справочник — сверить не с чем;            │
                # │             9 справочник знает и спорит (Balg, Bongo, Chino,       │
                # │               Cluster, Insektenspray, Pastrami, RAID, Silo,        │
                # │               Streusel) — все девять на СНЯТЫХ строках, в игру     │
                # │               не идут, и часть по-немецки правда двухродовая.      │
                # │                                                                   │
                # │ РЕШЕНИЕ ВЛАДЕЛЬЦА 25.08.2026, дословно по смыслу: «если род не    │
                # │ знает ни справочник, ни база, а знает только модель — то на кого   │
                # │ нам ещё надеяться? Это редкие слова, их почти никто не спросит.    │
                # │ Мы за этот ответ уже заплатили. Я снимал слово не потому, что не   │
                # │ согласен с артиклем, а потому что оно не нужно в тренировке.»      │
                # │                                                                   │
                # │ ОТСЮДА ДВА ВЫВОДА, КОТОРЫЕ МЕНЯТЬ НЕЛЬЗЯ БЕЗ ВЛАДЕЛЬЦА:           │
                # │  1. retired = «не нужно в тренировке» (редкое, неупотребимое).     │
                # │     Это НЕ «артикль забракован». Несогласие с самим написанием     │
                # │     живёт отдельно — в retire_reason (RETIRE_REASON_BAD_FORM),     │
                # │     и вот ОНО из выборки исключается, строкой ниже через bad_forms.│
                # │  2. Сузить выборку до verified значит обрушить покрытие арбитра    │
                # │     ради слов, которые почти никто не спрашивает. Не делать.       │
                # │                                                                   │
                # │ Перемерить, если понадобится: GROUP BY lower(word) HAVING          │
                # │ BOOL_AND(NOT verified), затем сверить с _article_from_declension.  │
                # └───────────────────────────────────────────────────────────────────┘
