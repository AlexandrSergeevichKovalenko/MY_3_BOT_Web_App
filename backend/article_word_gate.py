...
def judge_everyday_words(words: list[str]) -> dict[str, bool]:
    ...
    if not clean:
        return {}
    prompt = (
        "Ниже немецкие существительные. Для каждого ответь, встречается ли оно в "
        "обычной жизни: дома, на работе, в магазине, в дороге, у врача, в учёбе.\n"
        "ДА — если обычный человек видит, называет или использует это регулярно "
        "(Steckdose, Bohrmaschine, Wasserkocher, Quadrat, Ladekabel).\n"
        "НЕТ — если это узкий термин, редкость или книжное слово "
        "(Föhnsturm, Sturmtiefausläufer, Hygrometer, Impetigo).\n\n"
