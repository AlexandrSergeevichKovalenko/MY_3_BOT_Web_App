...
def shelf_target() -> int:
    """Сколько роликов держим в запасе. Семь — решение владельца 29.08.2026.

    Это НЕ основной источник рубрики: выпуск ищет ролик с колёс, как новости, и в запас
    лезет, только если поиск вернулся пустым. Семь штук — это неделя таких дней подряд;
    держать больше значит платить квотой и временем за склад, которым не пользуются.
    """
    return _env_int("STANDUP_SHELF_TARGET", 7)
...
def refill_standup_shelf(*, target: int | None = None, max_add: int | None = None,
    ...
    # Общего лимита «с потолка» здесь БОЛЬШЕ НЕТ (переделано 29.08.2026 по требованию
    # владельца: «зачем эти цифры придумывать с потолка, из-за которых потом нормальные
    # видео бракуются»). Предыдущие 150 секунд именно этим и кончились: работа успевала
    # один ролик из двенадцати и обрывалась на середине.
    #
    # Теперь предел ВЫЧИСЛЯЕТСЯ и означает ровно «худший случай, когда все ролики
    # окажутся неудачными»: сколько роликов берём за раз × таймаут одного ролика.
    # Ни одна дорога тут не оборвётся раньше, чем работа вправду упрётся в свой потолок.
    cap = int(max_add) if max_add is not None else _env_int("STANDUP_SHELF_MAX_ADD", 12)
    # ┌─ ПРОВЕРЕНО 29.08.2026 НА ЖИВОМ ПРОГОНЕ. НЕ УМЕНЬШАТЬ. ─────────────────────────┐
    # │ Таймаут одного ролика обязан быть ВЫШЕ полного прохода лестницы, иначе он режет │
    # │ не зависание, а ответ. Замер: удачный ролик отдаёт субтитры за 1–7 секунд,      │
    # │ полный отказ лестницы занимает 91–95 секунд. С таймаутом 90 c оба проверенных   │
    # │ ролика получили «не дождались» — временный вердикт вместо честного «субтитров   │
    # │ нет», приговор не выносился никогда, и через две недели мы жгли бы то же время  │
    # │ заново. 150 c — та же измеренная длина с запасом в полтора раза.                │
    # └───────────────────────────────────────────────────────────────────────────────┘
    item_timeout_sec = _env_int("STANDUP_SHELF_ITEM_TIMEOUT_SEC", 150)
    budget_sec = int(budget_sec) if budget_sec else cap * item_timeout_sec
