...
# Какая таблица на странице «наша» — по нашей части речи. Имена шаблонов принадлежат
# самому справочнику, мы их не придумываем.
_НАША_ТАБЛИЦА = {"noun": "Substantiv", "adjective": "Adjektiv", "adverb": "Adjektiv"}
# Человеческое имя чужой таблицы. Список открытый: чего здесь нет — назовём как есть.
_ЧУЖАЯ_ТАБЛИЦА = {
    "Nachname": "склонение ФАМИЛИИ (der Finster — господин Финстер, die Finsters — семья)",
    "Vorname": "склонение ИМЕНИ",
    "Toponym": "склонение географического названия",
    "Substantiv": "склонение существительного",
    "Adjektiv": "степени сравнения прилагательного",
    "Verb": "спряжение глагола",
}


def _таблицы_страницы(text: str) -> list[str]:
    """Какие таблицы напечатаны на странице: «Nachname», «Substantiv», «Verb»…"""
    found = re.findall(r"\{\{Deutsch ([^\n}|]*?) Übersicht", str(text or ""))
    return sorted({str(x).strip() for x in found if str(x).strip()})


def diagnose_source(pos: str, source_text: str | None) -> tuple[str, str]:
    """Почему формы не собрались — КОДОМ и человеческой фразой.

    ┌─ ПЕРЕДЕЛАНО 27.08.2026 ПО ЗАМЕЧАНИЮ ВЛАДЕЛЬЦА. НЕ ВОЗВРАЩАТЬ ОБЩУЮ ФРАЗУ. ─────┐
    │ В карточке стояло одно на все случаи: «справочник их не печатает». Владелец     │
    │ открыл страницу «Finster», увидел там таблицу и назвал это обманом — по делу.   │
    │ Таблица там есть, но напечатана шаблоном «Deutsch Nachname Übersicht»: это      │
    │ склонение ФАМИЛИИ, а у нас слово записано как «темнота». Взять её значило бы    │
    │ выучить человека немецкой фамилии вместо слова.                                 │
    │                                                                                 │
    │ Причина смотрится по ТАБЛИЦАМ страницы, а не только по пометке части речи:      │
    │ у «Finster» помечены и Substantiv, и Nachname, поэтому проверка «часть речи     │
    │ совпала» ничего не ловила. Владелец видит на экране именно таблицу — о ней и    │
    │ говорим.                                                                        │
    │                                                                                 │
    │ От кода зависит набор кнопок: настоящее слово без таблицы удалять нельзя,       │
    │ наше неподтверждённое написание — можно.                                        │
    └─────────────────────────────────────────────────────────────────────────────────┘
    """
    kind_ru = {"noun": "существительным", "adjective": "прилагательным",
               "adverb": "наречием"}.get(str(pos or "").strip().lower(), str(pos or ""))
    if source_text is None:
        return "молчит", "справочник сейчас не ответил — причину назову в следующий раз"
    text = str(source_text or "")
    if not text:
        return "нет_страницы", "страницы с таким написанием в справочнике нет"
    kinds = set(re.findall(r"\{\{Wortart\|([^|}]+)", text))
    if not _pos_matches(pos, kinds):
        какие = ", ".join(sorted(kinds)[:3])
        return ("другая_часть_речи",
                f"справочник знает это слово как {какие}, а у нас оно записано {kind_ru}")
    таблицы = _таблицы_страницы(text)
    наша = _НАША_ТАБЛИЦА.get(str(pos or "").strip().lower(), "")
    if наша and наша not in таблицы:
        if таблицы:
            чужая = таблицы[0]
            что = _ЧУЖАЯ_ТАБЛИЦА.get(чужая, f"таблица «{чужая}»")
            код = "только_фамилия" if чужая in ("Nachname", "Vorname") else "чужая_таблица"
            return (код, f"на странице напечатана НЕ таблица нашего слова, а {что} — "
                         f"к нашему значению она не относится")
        return ("нет_таблицы",
                "страница есть и часть речи та же, но таблицы форм на ней не напечатано")
    return ("нет_таблицы",
            "страница есть и часть речи та же, но таблицы форм на ней не напечатано")
...
# появиться. Но и часто спрашивать бессмысленно: статьи в справочнике не появляются
# за ночь.
#
# Владелец 27.08.2026, дословно: «если она появится завтра? ну так тогда нужно делать
# запрос через месяц хотя бы, но точно не завтра, один раз в месяц запрашивать».
_RECHECK_NEGATIVE_AFTER_DAYS = 30
