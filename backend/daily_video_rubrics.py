...
STANDUP_PROFILE = RubricProfile(
    ...
    min_phrases=4,
    max_phrases=40,
    # ┌─ 29.08.2026: БЫЛО 8 (до 400 роликов с канала). НЕ ВОЗВРАЩАТЬ БЕЗ НУЖДЫ. ──────┐
    # │ Восемь страниц — это обход всего архива: 3764 ролика и ~152 единицы квоты за    │
    # │ каждый проход. Он был нужен, пока полка была единственным источником и её       │
    # │ требовалось держать полной на 30 роликов. Теперь выпуск ищет ролик с колёс, как │
    # │ новости, и ему нужен ОДИН годный ролик, а запас — семь штук на аварию.          │
    # │ Одна страница на канал: 12 каналов ≈ 24 единицы квоты вместо 152.               │
    # │ 06.09.2026: свежий срез так и остался одной страницей. Если он пуст (всё      │
    # │ показано/осуждено), вечерний поиск докупает по ОДНОЙ странице глубже, по       │
    # │ токенам, до STANDUP_ARCHIVE_MAX_PAGES — см. _pick_video_with_transcript.       │
    # └───────────────────────────────────────────────────────────────────────────────┘
    archive_pages=1,
