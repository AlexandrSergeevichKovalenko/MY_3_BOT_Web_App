...
ADMIN_COMMAND_TOPICS: list[tuple[str, str, list[dict]]] = [
    ...
    ('econ', '📊 Аналитика, экономика, бюджеты', [
        ...
        {
            "cmd": '/ttsbudget',
            "desc": 'Панель-«рубильник» бюджета озвучки/перевода. Кнопки: 🛑 СТОП вся озвучка (блокирует разом все три корзины TTS) и по отдельности — ⛔ WaveNet (дорогой голос словаря/SRS/игр/новостей — тот, что тратит деньги сверх бесплатного лимита), ⛔ Standard (дешёвый голос классики), ⛔ платная озвучка книг, ⛔ Translate; плюс увеличение лимита и отправка месячного отчёта. Блокировка мгновенно останавливает синтез до обращения к Google и переживает перезапуск.',
            "args": 'Подкоманда (первый аргумент): status (по умолчанию), sendnow, add <число_символов>, block/unblock (WaveNet), std_block/std_unblock (Standard-классика), paid_block/paid_unblock (платная озвучка), а также варианты с префиксом translate_ (например translate_add) для Google Translate; для add вторым аргументом идёт количество символов. Кнопки «СТОП/Включить всю озвучку» доступны только из панели.',
            "example": '/ttsbudget std_block',
        },
        {
            "cmd": '/budgets',
