...
ADMIN_COMMAND_TOPICS: list[tuple[str, str, list[dict]]] = [
    ...
    ('schedule', '🗓 Планировщик, рассылки, здоровье', [
        ...
        },
        {
            ...
            "desc": 'Анонс «пройди тур по боту». Без аргументов — превью себе. С «send» — разослать всем разрешённым, кто ещё НЕ прошёл онбординг и кому анонс не отправлялся (безопасно для повторного запуска). Параллельно бот 2 раза в день мягко напоминает непрошедшим (кнопка «Позже» отключает напоминания).',
