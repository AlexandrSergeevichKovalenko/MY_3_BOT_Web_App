...
ADMIN_COMMAND_TOPICS: list[tuple[str, str, list[dict]]] = [
    ...
    ('pools', '📦 Пулы и отчёты', [
        ...
        {
            "cmd": '/admin_pin_review',
            "desc": 'Открывает приёмку «Finde im Bild»: в Mini-App показывается картинка с черновой рамкой от модели, ты обводишь предмет пальцем — твоя рамка становится эталоном для тапа ученика и помечается как проверенная человеком (автоматика её больше не трогает). Новые задания создаются сразу на приёмке и людям не уходят, пока не примешь.',
            "args": 'без аргумента — только ждущие проверки; all — отправить на проверку ВСЕ живые pin-задания (формат встанет на паузу, пока не разберёшь).',
            "example": '/admin_pin_review all',
        },
        {
            "cmd": '/admin_pin_words',
            "desc": 'Задаёт слова для «Finde im Bild». Без аргументов — показывает размер очереди и просит прислать слова ответом; с аргументами — добавляет их (повторы и уже использованные отбрасываются) и запускает генерацию картинок. Когда слова кончаются, бот сам присылает запрос на новую партию.',
            "args": 'необязательно — слова через пробел (можно с артиклем): der Locher Wasserkocher die Sicherung.',
            "example": '/admin_pin_words der Locher Wasserkocher die Sicherung',
        },
        {
            "cmd": '/admin_aq_pool',
