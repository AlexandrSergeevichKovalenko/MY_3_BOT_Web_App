...
# Learning steps: Again=10min, Hard≈35min, Good@step0=1h, Good@step1=1d, Easy→graduate~2d
_LEARNING_STEPS = [timedelta(minutes=10), timedelta(hours=1), timedelta(days=1)]
...
def schedule_review(
    *,
    ...
    prior_status = _status_from_state(card.state)
    raw_lapses = int(getattr(reviewed_card, "lapses", 0) or 0)
    # py-fsrs does not increment lapses itself; increment when a review/relearning
    # card is rated Again so analytics and DB state reflect real lapse count.
    if rating_enum == Rating.Again and prior_status in ("review", "relearning"):
        raw_lapses = max(raw_lapses, int(getattr(card, "lapses", 0) or 0) + 1)
