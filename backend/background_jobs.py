...
# ⚠ ОЧЕРЕДЬ БЕРЁТСЯ ИЗ ТЕХ, ЧТО ВОРКЕР В САМОМ ДЕЛЕ СЛУШАЕТ.
#
# ┌─ ПРОВЕРЕНО 28.08.2026. НОВОЕ ИМЯ ОЧЕРЕДИ ЗДЕСЬ НЕ ЗАВОДИТЬ. ──────────────────┐
# │ Сначала здесь стояло `queue_name="word_audit_apply"` — по виду безобидно, а по │
# │ делу задание не взял бы никто: у сервиса BACKGROUND_JOBS переменная            │
# │ DRAMATIQ_QUEUES содержит ЯВНЫЙ список из пятнадцати очередей, и новой в нём    │
# │ нет. Задание молча копилось бы в Redis, человек получал бы «Принято» и никогда │
# │ — сообщения о готовности. Проверено `railway variables --service               │
# │ BACKGROUND_JOBS --kv | grep DRAMATIQ_QUEUES`.                                  │
# │ Поэтому берём `scheduler_jobs`: она в списке, и на ней уже живёт ближайший     │
# │ родственник — `run_dictionary_dedupe_after_save_job`, такая же работа «после   │
# │ того, как человек отпущен».                                                    │
# │ Заводить своё имя МОЖНО, но только вместе с правкой DRAMATIQ_QUEUES в Railway  │
# │ — одно без другого означает тихо потерянные задания.                           │
# └──────────────────────────────────────────────────────────────────────────────┘
...
def run_word_audit_apply_job(user_id: int, decisions: list) -> None:
    ...
    ┌─ РЕШЕНИЕ ВЛАДЕЛЬЦА 31.08.2026. СООБЩЕНИЙ ЧЕЛОВЕКУ ОТСЮДА НЕ СЛАТЬ. ────────────┐
    │ Отсюда уходило два письма: «🦊 Проверка слов: готово. оставили как есть: 1 ·   │
    │ исправлено: 3 … карточки достроим этой ночью» при успехе и «не смог сохранить  │
    │ твои решения» при сбое. Оба убраны. Владелец 31.08.2026: «человек этим          │
    │ процессом не управляет, он заплатил деньги и в его картине мира всё уже         │
    │ сделано; зачем мы грузим его технической информацией».                          │
    │ Молчание здесь не прячет дефект, и вот почему:                                 │
    │  • подтверждение человек УЖЕ получил на экране — «Принято, твоих решений: N»;  │
    │  • сорванная работа не теряется: записи остаются открытыми и сами придут ему   │
    │    на проверку в ближайший вторник или пятницу (`send_word_audit_reminders`);  │
    │  • сбой виден в логах строкой `word_audit_apply_job failed` и наружу поднимает │
    │    исключение — он лечится повтором круга, а не письмом человеку.              │
    │ НЕ ВОЗВРАЩАТЬ сюда уведомление, не спросив владельца.                          │
    └───────────────────────────────────────────────────────────────────────────────┘
    """
    safe_user_id = int(user_id or 0)
    ...
    try:
        ...
        # Наружу поднимаем: отметки при этом не потеряны — записи остаются открытыми
        # и придут человеку на проверку следующим вторником или пятницей.
...
@dramatiq.actor(max_retries=0, queue_name="scheduler_jobs")
def run_stars_refund_weekly_report_actor() -> None:
    from backend.backend_server import _run_stars_refund_weekly_report_job
    _run_stars_refund_weekly_report_job()
...
@dramatiq.actor(max_retries=0, queue_name="scheduler_jobs")
def run_translation_check_stale_cleanup_actor() -> None:
...
@dramatiq.actor(max_retries=0, queue_name="scheduler_jobs")
def run_admin_economics_report_actor() -> None:
    from backend.admin_economics import send_admin_economics_report
...
def run_verb_paradigm_warm_actor() -> None:
    ...
    Пока парадигма глагола не спрошена, таблицы НЕТ ВОВСЕ: считать её от основы
    запрещено (решение владельца 23.08.2026 — счёт давал «ich boree», «ich besagte»).
    Порция маленькая: справочник уходит в лимит примерно на десятом запросе подряд.

    Вторым шагом тот же прогон добирает моделью глаголы, которых в справочнике нет:
    два спроса, ответ принимается только при полном совпадении. Числа `model_*` в
    логе — сколько спросили, сколько подтвердилось и сколько осталось без таблицы."""
    ...
    # СВЕРКА ИДЁТ СРАЗУ ЗА ПРОГРЕВОМ, В ТОМ ЖЕ ПРОГОНЕ. Не отдельной задачей в соседний
    # час: порядок «сначала справочник вырос, потом сверили» обязан быть гарантирован,
    # а не угадан по времени.
    #
    # Почему это вообще нужно каждую ночь (владелец, 02.09.2026). Кривой указатель формы
    # («ging» → ausgehen) удаляется только тогда, когда справочник знает базовый глагол и
    # может это ДОКАЗАТЬ. Пока не знает — указатель живёт. Справочник растёт по 200
    # глаголов за ночь, значит и доказуемых указателей каждую ночь становится больше.
    # Без этой строчки вышло бы то, что владелец запретил: «через одиннадцать ночей
    # справочник дорастёт, а указатели так и останутся кривыми, пока кто-то не вспомнит».
    try:
        from backend.lex_form_index_sweep import sweep_form_index
        sweep = sweep_form_index(apply=True)
        logging.info("указатели словоформ: сверка %s", sweep)
    except Exception:
        # Сверка — вторая половина ночной работы, но она НЕ должна утащить за собой
        # прогрев: он уже отработал и записал таблицы. Ошибку логируем целиком, тихого
        # «всё хорошо» здесь нет — счётчик снятого просто не вырастет, и это будет
        # видно в недельной строчке владельцу.
        logging.exception("указатели словоформ: сверка не прошла")
...
def run_lex_form_index_report_actor() -> None:
    ...
    Владелец 19.08.2026: «всё, что я должен вызывать командой, я забуду». Поэтому отчёт
    приходит сам — молчащий механизм неотличим от сломанного."""
    from backend.lex_form_index_sweep import send_form_index_report
    result = send_form_index_report()
    logging.info("указатели словоформ: отчёт %s", result)
...
def run_reference_forms_warm_actor() -> None:
    ...
    # ПРИГОВОР, НЕ ДОЕХАВШИЙ ДО ДАННЫХ, ПОДБИРАЕТСЯ ЗДЕСЬ ЖЕ, СЛЕДУЮЩЕЙ НОЧЬЮ.
    #
    # Проход выше берёт только НОВЫЕ слова: приговор ложится в кеш до применения, и с
    # этой секунды слово для него больше не существует. Сорвалось применение — второй
    # попытки не было НИКОГДА и молча. Замер 25.08.2026: так зависли 47 приговоров, 38
    # «исправлено» и 9 «не слово», часть с 19 августа.
    #
    # Здесь они переспрашиваются у ЖИВОЙ двери и применяются. Переспрос обязателен:
    # дверь с тех пор чинили, и семь приговоров из тридцати восьми оказались ядовитыми
    # («gehen» → «Gehen», «vier» → «Vier») — сегодня она сама говорит «не трогать».
    from backend.word_gate_apply import apply_pending_verdicts
    logging.info("дверь слова: применение приговоров %s", apply_pending_verdicts(
        apply=(_os.getenv("WORD_GATE_APPLY_VERDICTS") or "1").strip() == "1"))

    # ┌─ ТРИ ЧИНЯЩИХ МЕХАНИЗМА, КОТОРЫЕ БЫЛИ НАПИСАНЫ И НЕ ЗАПУСКАЛИСЬ НИКЕМ ─────────┐
    # │ Владелец 25.08.2026: «мы превратили это в бесконечный процесс — ты каждый день │
    # │ находишь по два-три слова без рода, без связи, без ссылки».                    │
    # │                                                                               │
    # │ Причина найдена и она одна. Эти функции существуют давно и чинят ровно те      │
    # │ классы, которые я приносил владельцу руками. Проверка грепом 25.08.2026:       │
    # │ НИ ОДНА из них не вызывалась ни ночной работой, ни планировщиком, ни отчётом.  │
    # │ Ноль вызовов. Мусор копился, а выгребал его я — по два-три слова в день,       │
    # │ и конца этому не было по построению.                                           │
    # │                                                                               │
    # │ Все три бесплатные: ни сети, ни модели, только наши же данные.                 │
    # └───────────────────────────────────────────────────────────────────────────────┘
    from backend import lex_units as _lex
    # Род и часть речи из разбора — в поля слова. Без этого слово не попадает в игру
    # «Артикли»: она читает поле, а не текст разбора. Замер 25.08.2026: 4 слова.
    logging.info("ночная починка: род и часть речи из разбора %s",
                 _lex.backfill_pos_gender_from_cards(
                     limit=int((_os.getenv("POS_GENDER_BACKFILL_BATCH") or "500").strip() or "500")))
    # Род лежит колонкой и врёт на экране — сверка с источником рода.
    logging.info("ночная починка: спорный род по источнику %s",
                 _lex.fix_gender_conflicts_from_authority(
                     limit=int((_os.getenv("GENDER_CONFLICT_BATCH") or "400").strip() or "400")))
    # Связи «немецкое ↔ русское» из разбора. Пакетного прохода не существовало вовсе,
    # поэтому 67 слов накопились без связей и число росло. Замер 25.08.2026.
    logging.info("ночная починка: связи из разбора %s",
                 _lex.backfill_links_from_cards(
                     limit=int((_os.getenv("LINKS_BACKFILL_BATCH") or "300").strip() or "300")))
