    ...
    plugins: [
      ...
      VitePWA({
        ...
        workbox: {
          ...
          // SERVICE WORKER БОЛЬШЕ НЕ ОБСЛУЖИВАЕТ ОТКРЫТИЕ ПРИЛОЖЕНИЯ ВООБЩЕ.
          //
          // `navigateFallback: null` — то есть любой заход на страницу идёт на сервер, а не в
          // сохранённую копию. Это сильнее любых списков исключений и закрывает целый класс
          // бед: сломанная страница, один раз попавшая в кеш, залипала намертво, а изнутри
          // Telegram кеш почистить нельзя — пользователь оставался с пустым экраном, сколько
          // бы исправлений мы ни задеплоили. Ровно это и произошло.
          //
          // Офлайн от этого не страдает по существу: без сети приложение бесполезно, весь
          // контент приходит с сервера. Кеш ассетов (скрипты, стили, шрифты) остаётся —
          // они с хешами в имени, залипнуть не могут.
          navigateFallback: null,
          // Список ниже теперь избыточен, но оставлен намеренно: если когда-нибудь вернут
          // navigateFallback, эти адреса обязаны ходить на сервер (см. причины ниже).
          //
