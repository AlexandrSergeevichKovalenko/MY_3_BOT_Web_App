...
export default defineConfig(async () => {
  ...
  return {
    ...
    plugins: [
      react(),
      VitePWA({
        // ┌─ ПРОВЕРЕНО 16.09.2026. ЭТОТ РЕЖИМ БОЛЬШЕ НИ НА ЧТО НЕ ВЛИЯЕТ. ─────────────────┐
        // │ registerType задаёт поведение ТОЛЬКО виртуального модуля 'virtual:pwa-register',│
        // │ а мы его больше не подключаем: worker регистрирует наш собственный код          │
        // │ (main.jsx), потому что в обоих режимах плагин вписывал в этот модуль свою       │
        // │ безусловную window.location.reload() поверх нашего стража. Оставлено 'prompt'   │
        // │ как менее агрессивное значение — на случай, если модуль когда-нибудь вернут.    │
        // │ Сам sw.js от режима не зависит: skipWaiting и clientsClaim заданы ниже явно.    │
        // └────────────────────────────────────────────────────────────────────────────────┘
        registerType: 'prompt',
