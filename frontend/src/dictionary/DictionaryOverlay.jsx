...
export default function DictionaryOverlay({ onClose, sharedDiffToken = '' } = {}) {
  ...
  // ┌─ ПАНЕЛЬ = ВИДИМАЯ ПОЛОСА ОКНА. ТРИ ПОПЫТКИ 16.09.2026, ЧИТАТЬ ВСЕ ТРИ. ──────────┐
  // │ Жалоба: встаёшь в поле — экран дёргается, шапка уезжает, «Перевести» вылезает.   │
  // │ Причина у всего одна: айфон при выезде клавиатуры окно СТРАНИЦЫ не уменьшает.    │
  // │ Он ПОДКРУЧИВАЕТ страницу вверх, чтобы поле оказалось над клавиатурой, и решение  │
  // │ это принимает ПО СТАРОЙ раскладке — до того, как мы успели что-либо уменьшить.   │
  // │ Отменять свою подкрутку он потом не станет.                                      │
  // │                                                                                  │
  // │ Попытка 1 — слой position: fixed ПЛЮС ручной сдвиг на visualViewport.offsetTop.  │
  // │   Карточка съезжала вниз и возвращалась. Разбор: WebKit фиксированный слой уже   │
  // │   пересадил на видимую полосу САМ, наш сдвиг сложился с его. Доказательство —    │
  // │   карточка оказывалась НИЖЕ правильного места ровно на величину компенсации.     │
  // │ Попытка 2 — убрал и сдвиг, и сам fixed: панель в обычном потоке.                 │
  // │   Стало хуже всего: подкручивать страницу теперь ничто не мешало, экран уехал     │
  // │   вверх на ~280 точек, шапки и вкладок не осталось, снизу серая пустота          │
  // │   (видео владельца 12-53-06, кадр 30). 17 движущихся кадров из 115.              │
  // │ Попытка 3, здесь — fixed БЕЗ компенсации, плюс запертая страница.                │
  // │   Слой стоит в видимой полосе (WebKit сам его туда и сажает), а странице нечего  │
  // │   подкручивать: html и body заперты. Меряем ОДНО число — высоту.                 │
  // │                                                                                  │
  // │ ⛔ offsetTop сюда не возвращать (попытка 1). ⛔ fixed отсюда не убирать (попытка   │
  // │ 2). Обе ошибки уже стоили владельцу двух заходов и записаны в обещание           │
  // │ quick_dict_keyboard_never_drags_the_screen как запрещённые метки.                │
  // │ Перемерить: ffmpeg -vf fps=10 по видео + покадровая разница верхней трети кадра  │
  // │ (порог 1% точек). Цель — 0 движущихся кадров, столько же, сколько у Яндекса.     │
  // └──────────────────────────────────────────────────────────────────────────────────┘
  useEffect(() => {
  ...
  const translate = useCallback(async (overrideText) => {
    ...
    try {
      ...
      // │ неполным. Мерили только БЕСПЛАТНУЮ половину фонового добора (прогрев +     │
      // │ справочник): 28–30 с на длинных сложных словах, артикль не найден ни у     │
      // │ одного из пяти. Но у «Kuddelmuddel» справочник ответил за 2,6 с, а вторую  │
      // │ половину — обращение к модели — не мерили ВООБЩЕ. Она может уложиться в    │
      // │ оставшиеся 5,7 с, и тогда опрос доносит артикль до экрана.                 │
      // │                                                                           │
      // │ Полный замер (с моделью) ведёт agent/gsurface — тема артикля целиком их.   │
      // │                                                                           │
      // │ УБРАТЬ ОПРОС МОЖНО ТОЛЬКО ПОСЛЕ ТОГО, как фоновый добор начнёт писать      │
      // │ найденный артикль в БАНК СЛОВ (bt_3_lex_units.gender), а не в память       │
      // │ процесса. Пока он пишет в память, опрос — единственный способ, которым     │
      // │ артикль доезжает до экрана после ответа. Правка в работе у agent/gsurface; │
      // │ они скажут, когда выкатят, и тогда опрос выключается одной строкой.        │
      // │                                                                           │
      // │ Что уже сделано и работает: опрос не запускается у слов, чья часть речи    │
      // │ известна и это НЕ существительное (см. germanNounAwaitingArticle) — минус  │
      // │ 42% всех опросов, замерено на 299 реальных запросах.                       │
      // └───────────────────────────────────────────────────────────────────────────┘
  ...
  //
  // 28.08.2026: оба срока увеличены на 30% (900→1170, 2200→2860). Решение владельца:
  // «давай дадим человеку времени подумать». Повод — разбор мусора в общем словаре:
  // «Spal» вместо «Spalte», «Anzeichen für einen Herzi» — это недописанные слова,
  // пойманные ранним переводом.
  //
  // ⚠ И СРАЗУ ЧЕСТНО: миллисекунды тут НЕ ЛЕЧАТ. Хоть три секунды поставь — человек
  // всё равно иногда задумается посреди слова, и обрезок всё равно родится. Срок
  // делает попадание реже, а не невозможным. Настоящее лекарство не здесь, а в том,
  // что недописанное больше не уходит в ОБЩИЕ хранилища: туда попадает только то,
  // что человек подтвердил кнопкой «Сохранить». Не ищи здесь идеальное число — его
  // нет, и подбор его уже однажды съел время (09.08.2026, 800→900/2200).
  const autoDelayMs = query.trim().includes(' ') ? 2860 : 1170;
  useEffect(() => {
  ...
  const fitInputHeight = useCallback((el) => {
    ...
    // ┌─ 16.09.2026. БОЛЬШОЕ ПОЛЕ ТЕПЕРЬ ТОЖЕ РАСТЁТ ПО ТЕКСТУ. ЭТО НЕ КОСМЕТИКА. ────────┐
    // │ Раньше здесь стояло «большое поле не трогаем, оно должно занимать пол-экрана».   │
    // │ Выглядит оно так и сейчас — пол-экрана белого, — но БЕЛОЕ МЕСТО теперь не само   │
    // │ поле, а пустая подложка под ним (.dq-compose-fill), и тап по ней ставит курсор   │
    // │ в поле. Разница невидима глазу и решающая для айфона.                            │
    // │ Айфон, когда в поле ставят курсор, подкручивает страницу так, чтобы ПОЛЕ было    │
    // │ видно целиком. Поле высотой в пол-экрана нижним краем уходит под клавиатуру —    │
    // │ значит подкрутка неизбежна, и обойти её нельзя ничем: ни слоями, ни запертой     │
    // │ прокруткой (проверено тремя способами 16.09.2026, все три оставляли прыжок).     │
    // │ Поле высотой в одну строку у верха экрана видно и так — подкручивать нечего.     │
    // │ Ровно поэтому у Яндекс.Переводчика на покадровом замере 0 движущихся кадров из   │
    // │ 63: у него поле тоже растёт по тексту, а большая белая область — подложка.       │
    // └──────────────────────────────────────────────────────────────────────────────────┘
    if (!el || el.tagName !== 'TEXTAREA') return;
    if (!el.classList.contains('dq-input--multi') && !el.classList.contains('dq-textarea')) return;
    el.style.height = 'auto';
    el.style.height = `${el.scrollHeight}px`;
  }, []);

  // Тап по пустой белой подложке — это тап по полю: курсор встаёт в конец текста.
  // preventDefault нужен, чтобы браузер не снял фокус с поля ещё до нашего focus();
  // сам focus() остаётся ВНУТРИ жеста человека, иначе телефон клавиатуру не откроет
  // (проверено 15.09.2026, см. комментарий про автофокус выше).
  const focusComposeInput = useCallback((e) => {
    e.preventDefault();
    const el = inputRef.current;
    if (!el) return;
    el.focus();
    try { el.setSelectionRange(el.value.length, el.value.length); } catch (_e) { /* поле без выделения */ }
  }, []);
        ...
        {!quick ? (
          ...
          <div className="dq-compose">
            <textarea
              ...
              autoComplete="off"
              /* rows=1 — это НЕ внешний вид, а высота, которую поле требует себе как
                 минимум. По умолчанию у textarea две строки: пустое поле занимало 76
                 точек вместо 50, и на экране с клавиатурой этих 26 точек не хватало.
                 Выглядит поле по-прежнему большим: белое место под ним даёт подложка. */
              rows={1}
              ...
              onKeyDown={onKeyDown}
            />
            {/* Пустая подложка под полем: она и даёт «поле на пол-экрана» на вид, а тап по
                ней ставит курсор в поле. Само поле при этом ростом в свой текст — только
                так айфон не подкручивает экран при появлении клавиатуры (разбор выше,
                у fitInputHeight). */}
            <div className="dq-compose-fill" onMouseDown={focusComposeInput} aria-hidden="true" />
