...
// ┌──────────────────────────────────────────────────────────────────────────────────┐
// │ ПЕРЕПИСЬ 13–14.09.2026: что именно мешает снять эту строку.                      │
// │                                                                                  │
// │ Строка выглядит как выключатель, но английский включается НЕ ею. Перепись        │
// │ нашла четыре препятствия, и порядок у них такой:                                 │
// │                                                                                  │
// │  1. КАРТОЧКА УСТРОЕНА ПОД НЕМЕЦКИЙ. Личная карточка хранит слово в колонках      │
// │     word_de / translation_de. У всех 274 английских карточек они ПУСТЫ (замер    │
// │     живой базы 13.09.2026), английский лежит только в response_json.source_text. │
// │     Поиск и показ читают колонки — значит английского на экране просто нет.      │
// │  2. ЗАСЛОН ЕСТЬ ТОЛЬКО В ТРЕНАЖЁРЕ. Очередь повторения сверяет пару карточки     │
// │     (database.py, get_next_due_srs_card). Ручной отбор его выключает намеренно.  │
// │     Остальные экраны показывают всё подряд — сегодня это верно, при двух языках  │
// │     начнут смешивать.                                                            │
// │  3. ОБЩАЯ ДОРОЖКА ПРЕДЛОЖЕНИЙ языка не проверяет вовсе, и сегодня её спасают     │
// │     две случайности, а не правило (database.py, pick_words_for_sentence_track).  │
// │  4. ПАРЫ en↔de НЕТ НИ ОДНОЙ. Замер 13.09.2026: связей en↔ru — 261, en↔de — 0.    │
// │                                                                                  │
// │ Что ПРОВЕРЕНО и больше не мешает: спринт артиклей личные карточки не читает      │
// │ вовсе, поэтому старая тревога «английское слово без рода попадёт в спринт»       │
// │ снята (backend/article_sprint_generator.py, там же доказательство).              │
// │                                                                                  │
// │ За месяц 10.08 → 13.09.2026 английский в базе не сдвинулся: 274 карточки,        │
// │ 255 единиц, последняя английская карточка — 21.05.2026. Профилей 20, все ru/de.  │
// └──────────────────────────────────────────────────────────────────────────────────┘
