...
const ИМЕНА_ПОЛЕЙ = {
  // Заголовок слова: в разборе он живёт под тремя разными именами, а человеку это
  // одно и то же — само слово.
  word_de: 'Слово',
  source_text: 'Слово',
  translation_de: 'Слово',
  article: 'Артикль',
  word_ru: 'Перевод',
...
// Пустое значение в предложении модели значит «поле убрать», и кнопка обязана называть
// именно это действие. «Заменить формы» на предложении {forms: {}} — обещание, которого
// не будет: заменять там нечем.
function пусто(значение) {
  if (значение === null || значение === undefined || значение === '') return true;
  if (Array.isArray(значение)) return значение.length === 0;
  if (typeof значение === 'object') return Object.keys(значение).length === 0;
  return false;
}

function подписьДействия(поля) {
  const убрать = поля.filter(([, v]) => пусто(v)).map(([k]) => имяПоля(k).toLowerCase());
  const заменить = поля.filter(([, v]) => !пусто(v)).map(([k]) => имяПоля(k).toLowerCase());
  const части = [];
  if (заменить.length) части.push(`Заменить ${заменить.join(' и ')}`);
  if (убрать.length) части.push(`${части.length ? 'и убрать' : 'Убрать'} ${убрать.join(' и ')}`);
  return части.join(' ');
}
          ...
          <div className="wa-card cr-card" data-state={chosen} key={it.id}>
            ...
            {Object.keys(it.renames || {}).length ? (
              <div className="cr-block cr-rename">
                <div className="cr-block-title">Модель предлагает поменять само слово</div>
                {Object.entries(it.renames).map(([ключ, новое]) => {
                  // «Было» для заголовка берём из САМОГО СЛОВА, а не из поля разбора:
                  // у пустой карточки поля нет вовсе, и строка вышла бы полупустой —
                  // «: → die Nase putzen». Слово же есть всегда.
                  const было = читаемо((it.before || {})[ключ]) || (
                    ключ === 'article' ? '' : it.word);
                  return (
                    <p className="cr-line" key={ключ}>
                      {имяПоля(ключ)}:{' '}
                      {было ? <s className="cr-was-inline">{было}</s> : <i className="cr-empty">не указан</i>}
                      {' → '}<b>{читаемо(новое)}</b>
                    </p>
                  );
                })}
                <p className="cr-line cr-dim cr-diff-note">
                  Кнопка «Переименовать слово» внизу меняет только шапку. Если и разбор
                  не подходит новому слову — берите «Пересобрать всю карточку».
                </p>
              </div>
            ) : null}
                  ...
                  return (
                    <div className="cr-field" key={ключ}>
                      ...
                      <p className="cr-will">
                        ...
                        {/* ⚠ ПУСТОЕ ПРЕДЛОЖЕНИЕ — ЭТО ТОЖЕ ПРЕДЛОЖЕНИЕ: «поле убрать».
                            Владелец 27.08.2026 увидел на «Finster» слово «станет» и
                            пустоту после него: модель предложила убрать таблицу форм
                            ({"{"}forms: {"{}"}{"}"}), а экран напечатал ничего. Решение
                            принимают по этой строке — она обязана говорить, что будет. */}
                        {станет || <i className="cr-empty">пусто — поле уберём из карточки</i>}
                      </p>
                    </div>
                  );
                })}
