        ...
        return (
          <div className="wa-card cr-card" data-state={chosen} key={it.id}>
            ...
            <div className="cr-block">
              ...
              {поля.map(([имя, значение]) => (
                <p className="cr-line cr-dim" key={имя}>
                  <b>{имя}:</b> {typeof значение === 'string' ? значение : JSON.stringify(значение)}
                </p>
              ))}
            </div>
            ...
            <div className="wa-actions">
              {поля.length ? (
                <button type="button" className="wa-act cr-act-accept"
                  ...
                  {chosen === ПРИНЯТЬ ? '✓ ' : ''}Принять вариант
                </button>
              ) : null}
              <button type="button" className="wa-act"
                      onClick={() => pick(it.id, ПЕРЕСОБРАТЬ)}>
                {chosen === ПЕРЕСОБРАТЬ ? '✓ ' : ''}♻️ Пересобрать разбор
              </button>
