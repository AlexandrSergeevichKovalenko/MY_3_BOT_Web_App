...
async function bootstrapApp() {
  ...
  // ⛔ ГОСТЮ ПО ССЫЛКЕ «ПОДЕЛИТЬСЯ» ДВЕРЬ НЕ ПОКАЗЫВАЕМ.
  //
  // Решение владельца 28.08.2026: «пускать смотреть слово всегда, а очередь показывать,
  // только когда он захочет большего». Человек пришёл по ссылке на конкретное слово —
  // он ещё не знает ни про бота, ни про приложение, и звать его в очередь, не показав
  // ради чего, бессмысленно. Сначала он видит разбор, потом получает приглашение, и
  // только нажав его — попадает к боту и узнаёт про очередь.
  //
  // Разбор уже посчитан и отдаётся только на чтение — нам это не стоит ничего.
  const гостьПоСсылке = /^(share_|wdiff_)/i.test(answerStartParam);
  if (!гостьПоСсылке) {
