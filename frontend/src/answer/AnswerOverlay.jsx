...
function parseStartParam(startParam) {
  const m = /^ans_(rb|cw|ag|ls|qf|qfp|sp|au|asp|as)_(\d+)$/.exec(String(startParam || '').trim().toLowerCase());
  if (!m) return null;
...
export default function AnswerOverlay({ startParam }) {
  ...
  useEffect(() => {
    ...
    if (parsed.kind === 'sp' || parsed.kind === 'as' || parsed.kind === 'asp') { setMetaLoading(false); return; }  // these games load themselves
    let cancelled = false;
    (async () => {
      try {
