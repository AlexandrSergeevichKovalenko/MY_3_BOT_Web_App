...
function parseStartParam(startParam) {
  const m = /^ans_(rb|cw|ag|ls|qf|qfp|sp|au|mc|asbl|asb|asp|as|alf|al|rv|adbl|adb|adl|ad|bh|nd)_(\d+)$/.exec(String(startParam || '').trim().toLowerCase());
  if (!m) return null;
...
export default function AnswerOverlay({ startParam }) {
  ...
  useEffect(() => {
    ...
    if (['sp', 'as', 'asp', 'asb', 'asbl', 'al', 'alf', 'rv', 'ad', 'adb', 'adbl', 'adl', 'bh', 'nd'].includes(parsed.kind)) { setMetaLoading(false); return; }  // these games load themselves
    let cancelled = false;
    (async () => {
      try {
  ...
  if (kind === 'asp') {
    return <ArtikelSprintGame practice api={api} haptic={haptic} onClose={close} />;
  }
  if (kind === 'asb' && parsed?.id != null) {
    return <ArtikelSprintGame battleId={parsed.id} api={api} haptic={haptic} onClose={close} />;
  }
  if (kind === 'asbl') {
  ...
  if (kind === 'ad') {
    return <AdjektivSprintGame api={api} haptic={haptic} onClose={close} />;
  }
  if (kind === 'adb' && parsed?.id != null) {
    return <AdjektivSprintGame battleId={parsed.id} api={api} haptic={haptic} onClose={close} />;
  }
  if (kind === 'adbl') {
    return <AdjektivSprintGame battleList api={api} haptic={haptic} onClose={close} />;
  }
  if (kind === 'adl') {
