...
function parseStartParam(startParam) {
  const m = /^ans_(rb|cw|ag|ls|qf|qfp|sp|au|asbl|asb|asp|as|alf|al|rv|ad)_(\d+)$/.exec(String(startParam || '').trim().toLowerCase());
  if (!m) return null;
...
export default function AnswerOverlay({ startParam }) {
  ...
  useEffect(() => {
    ...
    if (['sp', 'as', 'asp', 'asb', 'asbl', 'al', 'alf', 'rv', 'ad'].includes(parsed.kind)) { setMetaLoading(false); return; }  // these games load themselves
    let cancelled = false;
    (async () => {
      try {
