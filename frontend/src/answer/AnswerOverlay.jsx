...
function parseStartParam(startParam) {
  const m = /^ans_(rb|cw|ag|ls|qf|qfp|sp|au|asp|asb|as)_(\d+)$/.exec(String(startParam || '').trim().toLowerCase());
  if (!m) return null;
...
export default function AnswerOverlay({ startParam }) {
  ...
  useEffect(() => {
    ...
    if (['sp', 'as', 'asp', 'asb'].includes(parsed.kind)) { setMetaLoading(false); return; }  // these games load themselves
    let cancelled = false;
    (async () => {
      try {
