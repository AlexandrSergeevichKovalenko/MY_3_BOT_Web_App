    ...
    <div className={`pinw frrev-w${typing ? ' typing' : ''}`}>
      ...
      <div className="pinw-bar">
        ...
        {/* ⛔ КНОПКА НАЗЫВАЕТСЯ ТЕМ, ЧТО ДЕЛАЕТ (владелец 31.08.2026: «а что такое
            „отложить“? а что такое „оставить“?»). Второстепенные решения стоят ОДНОЙ
            плотной строкой: два ряда высоких кнопок съедали половину экрана, а главное
            на этом экране — сам разбор наверху (владелец 04.09.2026). */}
        <div className="frrev-row frrev-row-tight">
          ...
          <button className="ans-btn-ghost frrev-closebtn" onClick={onClose}>✕</button>
        </div>
        {/* ⛔ ЗДЕСЬ БЫЛИ ТРИ СТРОКИ ПОДПИСЕЙ ПОД КНОПКАМИ. УБРАНЫ 04.09.2026 ПО ПРЯМОМУ
            УКАЗАНИЮ ВЛАДЕЛЬЦА и обратно не возвращаются:
            «убирай это на хуй с экрана, зачем ты тратишь 20% экрана на текст, который
            просто не нужен? это же телефон, здесь каждый сантиметр важен».
            Смысл кнопок теперь несут САМИ названия — «Всё верно», «Позже», «Удалить».
            Если названия перестанут говорить сами за себя, чинить надо названия, а не
            возвращать абзац под ними. */}
      </div>
    </div>
  );
}
