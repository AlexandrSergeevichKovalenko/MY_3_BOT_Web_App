...
// Высота, которой мы реально располагаем. Правило дословно как в fitCard.js — расхождение
// означало бы, что интерактив и модалка считают экран по-разному:
// развёрнутый мини-апп (штатное состояние, `expand()` при старте) — правду знает браузер;
// свёрнутый в шторку — видимую часть знает только `viewportStableHeight` Telegram.
// Подробное объяснение, почему выбор именно такой, — в комментарии к fitCard.js.
